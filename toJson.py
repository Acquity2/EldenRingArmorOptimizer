import json
from ArmorData import chest, helm, gauntlet, leg
import tkinter.messagebox as msgbox

chestText = chest.text()
ChestList = []

LegText = leg.text()
LegList = []

HelmText = helm.text()
HelmList = []

GauntletText = gauntlet.text()
GauntletList = []

text = ""

json1 = """
    
[{
    "Fir": 9.5,
    "Foc": 21.0,
    "Hol": 8.8,
    "Imm": 28.0,
    "Lit": 10.2,
    "Mag": 12.4,
    "Name": "All-Knowing Armor",
    "Phy": 12.9,
    "Poi": 16.0,
    "PoiPerWgt": 1.4953271028037385,
    "Robu": 46.0,
    "VSPie": 11.9,
    "VSSla": 13.5,
    "VSStr": 11.9,
    "Vita": 21.0,
    "Wgt": 10.7
},{
    "Fir": 11.9,
    "Foc": 67.0,
    "Hol": 12.8,
    "Imm": 38.0,
    "Lit": 12.4,
    "Mag": 12.8,
    "Name": "Alberich's Robe",
    "Phy": 5.3,
    "Poi": 1.0,
    "PoiPerWgt": 0.24390243902439027,
    "Robu": 23.0,
    "VSPie": 5.3,
    "VSSla": 5.3,
    "VSStr": 4.2,
    "Vita": 71.0,
    "Wgt": 4.1
},{
    "Fir": 11.4,
    "Foc": 42.0,
    "Hol": 8.8,
    "Imm": 50.0,
    "Lit": 10.9,
    "Mag": 10.9,
    "Name": "Aristocrat Coat",
    "Phy": 8.8,
    "Poi": 3.0,
    "PoiPerWgt": 0.4225352112676057,
    "Robu": 32.0,
    "VSPie": 8.8,
    "VSSla": 8.0,
    "VSStr": 8.8,
    "Vita": 46.0,
    "Wgt": 7.1
},{
    "Fir": 6.9,
    "Foc": 36.0,
    "Hol": 6.5,
    "Imm": 47.0,
    "Lit": 7.8,
    "Mag": 6.5,
    "Name": "Aristocrat Garb",
    "Phy": 7.8,
    "Poi": 2.0,
    "PoiPerWgt": 0.4081632653061224,
    "Robu": 33.0,
    "VSPie": 7.8,
    "VSSla": 7.8,
    "VSStr": 8.6,
    "Vita": 30.0,
    "Wgt": 4.9
},{
    "Fir": 13.0,
    "Foc": 90.0,
    "Hol": 13.0,
    "Imm": 50.0,
    "Lit": 13.3,
    "Mag": 13.5,
    "Name": "Astrologer Robe",
    "Phy": 6.7,
    "Poi": 5.0,
    "PoiPerWgt": 0.7936507936507937,
    "Robu": 24.0,
    "VSPie": 6.1,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 76.0,
    "Wgt": 6.3
},{
    "Fir": 12.8,
    "Foc": 99.0,
    "Hol": 13.8,
    "Imm": 50.0,
    "Lit": 13.0,
    "Mag": 15.4,
    "Name": "Azur's Glintstone Robe",
    "Phy": 7.1,
    "Poi": 3.0,
    "PoiPerWgt": 0.4225352112676057,
    "Robu": 28.0,
    "VSPie": 6.7,
    "VSSla": 6.7,
    "VSStr": 6.1,
    "Vita": 83.0,
    "Wgt": 7.1
},{
    "Fir": 10.2,
    "Foc": 60.0,
    "Hol": 9.5,
    "Imm": 57.0,
    "Lit": 10.9,
    "Mag": 10.2,
    "Name": "Bandit Garb",
    "Phy": 8.0,
    "Poi": 5.0,
    "PoiPerWgt": 0.6493506493506493,
    "Robu": 35.0,
    "VSPie": 9.5,
    "VSSla": 9.5,
    "VSStr": 8.8,
    "Vita": 60.0,
    "Wgt": 7.7
},{
    "Fir": 13.5,
    "Foc": 38.0,
    "Hol": 13.3,
    "Imm": 60.0,
    "Lit": 13.0,
    "Mag": 13.5,
    "Name": "Banished Knight Armor",
    "Phy": 18.7,
    "Poi": 28.0,
    "PoiPerWgt": 1.6,
    "Robu": 83.0,
    "VSPie": 17.5,
    "VSSla": 19.2,
    "VSStr": 15.0,
    "Vita": 42.0,
    "Wgt": 17.5
},{
    "Fir": 13.8,
    "Foc": 42.0,
    "Hol": 13.5,
    "Imm": 57.0,
    "Lit": 12.8,
    "Mag": 13.0,
    "Name": "Beast Champion Armor",
    "Phy": 17.5,
    "Poi": 28.0,
    "PoiPerWgt": 1.6,
    "Robu": 90.0,
    "VSPie": 18.3,
    "VSSla": 18.7,
    "VSStr": 16.3,
    "Vita": 38.0,
    "Wgt": 17.5
},{
    "Fir": 9.5,
    "Foc": 23.0,
    "Hol": 11.4,
    "Imm": 28.0,
    "Lit": 6.7,
    "Mag": 8.8,
    "Name": "Black Knife Armor",
    "Phy": 11.4,
    "Poi": 14.0,
    "PoiPerWgt": 1.5217391304347827,
    "Robu": 46.0,
    "VSPie": 12.4,
    "VSSla": 12.4,
    "VSStr": 10.9,
    "Vita": 23.0,
    "Wgt": 9.2
},{
    "Fir": 13.0,
    "Foc": 21.0,
    "Hol": 10.2,
    "Imm": 32.0,
    "Lit": 8.0,
    "Mag": 10.2,
    "Name": "Blackflame Monk Armor",
    "Phy": 13.5,
    "Poi": 19.0,
    "PoiPerWgt": 1.6101694915254237,
    "Robu": 55.0,
    "VSPie": 12.9,
    "VSSla": 14.6,
    "VSStr": 11.4,
    "Vita": 32.0,
    "Wgt": 11.8
},{
    "Fir": 10.2,
    "Foc": 23.0,
    "Hol": 10.3,
    "Imm": 32.0,
    "Lit": 7.1,
    "Mag": 9.5,
    "Name": "Bloodhound Knight Armor",
    "Phy": 12.4,
    "Poi": 14.0,
    "PoiPerWgt": 1.3207547169811322,
    "Robu": 50.0,
    "VSPie": 13.5,
    "VSSla": 14.0,
    "VSStr": 11.4,
    "Vita": 23.0,
    "Wgt": 10.6
},{
    "Fir": 10.9,
    "Foc": 50.0,
    "Hol": 9.5,
    "Imm": 63.0,
    "Lit": 11.9,
    "Mag": 10.2,
    "Name": "Blue Cloth Vest",
    "Phy": 9.5,
    "Poi": 6.0,
    "PoiPerWgt": 0.7792207792207793,
    "Robu": 42.0,
    "VSPie": 8.0,
    "VSSla": 8.0,
    "VSStr": 9.5,
    "Vita": 50.0,
    "Wgt": 7.7
},{
    "Fir": 10.2,
    "Foc": 18.0,
    "Hol": 8.0,
    "Imm": 25.0,
    "Lit": 8.0,
    "Mag": 10.9,
    "Name": "Blue Silver Mail Armor",
    "Phy": 12.4,
    "Poi": 14.0,
    "PoiPerWgt": 1.5217391304347827,
    "Robu": 60.0,
    "VSPie": 11.4,
    "VSSla": 12.9,
    "VSStr": 9.5,
    "Vita": 18.0,
    "Wgt": 9.2
},{
    "Fir": 12.4,
    "Foc": 24.0,
    "Hol": 10.9,
    "Imm": 35.0,
    "Lit": 8.8,
    "Mag": 10.9,
    "Name": "Briar Armor",
    "Phy": 12.9,
    "Poi": 15.0,
    "PoiPerWgt": 1.271186440677966,
    "Robu": 71.0,
    "VSPie": 12.4,
    "VSSla": 13.5,
    "VSStr": 11.4,
    "Vita": 24.0,
    "Wgt": 11.8
},{
    "Fir": 13.5,
    "Foc": 46.0,
    "Hol": 12.8,
    "Imm": 71.0,
    "Lit": 14.9,
    "Mag": 13.3,
    "Name": "Bull-Goat Armor",
    "Phy": 20.4,
    "Poi": 47.0,
    "PoiPerWgt": 1.7735849056603774,
    "Robu": 83.0,
    "VSPie": 18.3,
    "VSSla": 18.3,
    "VSStr": 20.2,
    "Vita": 55.0,
    "Wgt": 26.5
},{
    "Fir": 11.8,
    "Foc": 21.0,
    "Hol": 11.8,
    "Imm": 28.0,
    "Lit": 9.4,
    "Mag": 12.3,
    "Name": "Carian Knight Armor",
    "Phy": 11.8,
    "Poi": 12.0,
    "PoiPerWgt": 1.1538461538461537,
    "Robu": 46.0,
    "VSPie": 11.8,
    "VSSla": 12.3,
    "VSStr": 10.8,
    "Vita": 23.0,
    "Wgt": 10.4
},{
    "Fir": 10.9,
    "Foc": 11.0,
    "Hol": 8.0,
    "Imm": 25.0,
    "Lit": 6.1,
    "Mag": 7.1,
    "Name": "Chain Armor",
    "Phy": 11.9,
    "Poi": 12.0,
    "PoiPerWgt": 1.3636363636363635,
    "Robu": 42.0,
    "VSPie": 11.9,
    "VSSla": 12.9,
    "VSStr": 8.8,
    "Vita": 18.0,
    "Wgt": 8.8
},{
    "Fir": 11.4,
    "Foc": 55.0,
    "Hol": 10.9,
    "Imm": 63.0,
    "Lit": 11.9,
    "Mag": 10.9,
    "Name": "Chain-Draped Tabard",
    "Phy": 9.5,
    "Poi": 8.0,
    "PoiPerWgt": 0.9638554216867469,
    "Robu": 42.0,
    "VSPie": 9.5,
    "VSSla": 9.5,
    "VSStr": 10.2,
    "Vita": 55.0,
    "Wgt": 8.3
},{
    "Fir": 8.8,
    "Foc": 38.0,
    "Hol": 8.8,
    "Imm": 46.0,
    "Lit": 9.5,
    "Mag": 7.1,
    "Name": "Champion Pauldron",
    "Phy": 6.1,
    "Poi": 3.0,
    "PoiPerWgt": 0.5882352941176471,
    "Robu": 28.0,
    "VSPie": 7.1,
    "VSSla": 6.7,
    "VSStr": 8.0,
    "Vita": 35.0,
    "Wgt": 5.1
},{
    "Fir": 12.8,
    "Foc": 28.0,
    "Hol": 13.5,
    "Imm": 63.0,
    "Lit": 11.4,
    "Mag": 12.6,
    "Name": "Cleanrot Armor",
    "Phy": 14.6,
    "Poi": 23.0,
    "PoiPerWgt": 1.5333333333333334,
    "Robu": 67.0,
    "VSPie": 17.5,
    "VSSla": 16.0,
    "VSStr": 13.4,
    "Vita": 32.0,
    "Wgt": 15.0
},{
    "Fir": 12.4,
    "Foc": 60.0,
    "Hol": 12.4,
    "Imm": 42.0,
    "Lit": 11.9,
    "Mag": 12.8,
    "Name": "Cloth Garb",
    "Phy": 5.3,
    "Poi": 1.0,
    "PoiPerWgt": 0.3125,
    "Robu": 23.0,
    "VSPie": 4.2,
    "VSSla": 2.7,
    "VSStr": 4.2,
    "Vita": 60.0,
    "Wgt": 3.2
},{
    "Fir": 12.4,
    "Foc": 76.0,
    "Hol": 12.8,
    "Imm": 42.0,
    "Lit": 12.6,
    "Mag": 12.8,
    "Name": "Commoner's Garb",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 25.0,
    "VSPie": 6.1,
    "VSSla": 6.7,
    "VSStr": 6.7,
    "Vita": 83.0,
    "Wgt": 5.1
},{
    "Fir": 12.6,
    "Foc": 76.0,
    "Hol": 12.8,
    "Imm": 46.0,
    "Lit": 12.6,
    "Mag": 13.0,
    "Name": "Commoner's Simple Garb",
    "Phy": 4.2,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 24.0,
    "VSPie": 6.1,
    "VSSla": 6.7,
    "VSStr": 6.7,
    "Vita": 83.0,
    "Wgt": 5.1
},{
    "Fir": 11.4,
    "Foc": 50.0,
    "Hol": 10.2,
    "Imm": 63.0,
    "Lit": 12.4,
    "Mag": 11.4,
    "Name": "Confessor Armor",
    "Phy": 8.8,
    "Poi": 6.0,
    "PoiPerWgt": 0.7228915662650601,
    "Robu": 50.0,
    "VSPie": 9.5,
    "VSSla": 8.8,
    "VSStr": 10.9,
    "Vita": 55.0,
    "Wgt": 8.3
},{
    "Fir": 12.8,
    "Foc": 35.0,
    "Hol": 13.5,
    "Imm": 50.0,
    "Lit": 11.4,
    "Mag": 13.0,
    "Name": "Crucible Axe Armor",
    "Phy": 17.5,
    "Poi": 27.0,
    "PoiPerWgt": 1.7419354838709677,
    "Robu": 71.0,
    "VSPie": 16.8,
    "VSSla": 16.8,
    "VSStr": 13.9,
    "Vita": 35.0,
    "Wgt": 15.5
},{
    "Fir": 11.9,
    "Foc": 35.0,
    "Hol": 14.5,
    "Imm": 50.0,
    "Lit": 11.4,
    "Mag": 13.0,
    "Name": "Crucible Tree Armor",
    "Phy": 17.7,
    "Poi": 27.0,
    "PoiPerWgt": 1.7419354838709677,
    "Robu": 71.0,
    "VSPie": 15.8,
    "VSSla": 17.5,
    "VSStr": 13.9,
    "Vita": 35.0,
    "Wgt": 15.5
},{
    "Fir": 10.9,
    "Foc": 23.0,
    "Hol": 8.8,
    "Imm": 28.0,
    "Lit": 7.1,
    "Mag": 11.9,
    "Name": "Cuckoo Surcoat",
    "Phy": 12.9,
    "Poi": 16.0,
    "PoiPerWgt": 1.509433962264151,
    "Robu": 55.0,
    "VSPie": 12.9,
    "VSSla": 13.5,
    "VSStr": 10.9,
    "Vita": 23.0,
    "Wgt": 10.6
},{
    "Fir": 11.4,
    "Foc": 63.0,
    "Hol": 12.4,
    "Imm": 38.0,
    "Lit": 11.9,
    "Mag": 11.9,
    "Name": "Deathbed Dress",
    "Phy": 0.6,
    "Poi": 1.0,
    "PoiPerWgt": 0.3125,
    "Robu": 11.0,
    "VSPie": 0.6,
    "VSSla": 0.6,
    "VSStr": 2.7,
    "Vita": 107.0,
    "Wgt": 3.2
},{
    "Fir": 12.7,
    "Foc": 55.0,
    "Hol": 12.7,
    "Imm": 65.0,
    "Lit": 12.5,
    "Mag": 13.2,
    "Name": "Depraved Perfumer Robe",
    "Phy": 6.0,
    "Poi": 2.0,
    "PoiPerWgt": 0.4,
    "Robu": 22.0,
    "VSPie": 4.1,
    "VSSla": 5.2,
    "VSStr": 6.0,
    "Vita": 80.0,
    "Wgt": 5.0
},{
    "Fir": 10.9,
    "Foc": 23.0,
    "Hol": 9.5,
    "Imm": 35.0,
    "Lit": 7.1,
    "Mag": 10.9,
    "Name": "Eccentric's Armor",
    "Phy": 12.9,
    "Poi": 15.0,
    "PoiPerWgt": 1.4150943396226416,
    "Robu": 60.0,
    "VSPie": 12.9,
    "VSSla": 12.4,
    "VSStr": 10.9,
    "Vita": 21.0,
    "Wgt": 10.6
},{
    "Fir": 11.4,
    "Foc": 18.0,
    "Hol": 8.0,
    "Imm": 32.0,
    "Lit": 7.1,
    "Mag": 8.8,
    "Name": "Elden Lord Armor",
    "Phy": 11.9,
    "Poi": 14.0,
    "PoiPerWgt": 1.5217391304347827,
    "Robu": 55.0,
    "VSPie": 12.4,
    "VSSla": 11.4,
    "VSStr": 10.9,
    "Vita": 21.0,
    "Wgt": 9.2
},{
    "Fir": 10.2,
    "Foc": 23.0,
    "Hol": 9.5,
    "Imm": 32.0,
    "Lit": 8.8,
    "Mag": 9.5,
    "Name": "Erdtree Surcoat",
    "Phy": 12.9,
    "Poi": 16.0,
    "PoiPerWgt": 1.509433962264151,
    "Robu": 55.0,
    "VSPie": 12.9,
    "VSSla": 12.4,
    "VSStr": 11.9,
    "Vita": 23.0,
    "Wgt": 10.6
},{
    "Fir": 12.6,
    "Foc": 67.0,
    "Hol": 13.0,
    "Imm": 35.0,
    "Lit": 12.4,
    "Mag": 13.0,
    "Name": "Errant Sorcerer Robe",
    "Phy": 4.2,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 23.0,
    "VSPie": 4.2,
    "VSSla": 6.1,
    "VSStr": 4.2,
    "Vita": 71.0,
    "Wgt": 4.1
},{
    "Fir": 10.9,
    "Foc": 21.0,
    "Hol": 9.5,
    "Imm": 28.0,
    "Lit": 6.7,
    "Mag": 8.0,
    "Name": "Exile Armor",
    "Phy": 12.4,
    "Poi": 14.0,
    "PoiPerWgt": 1.5217391304347827,
    "Robu": 55.0,
    "VSPie": 12.4,
    "VSSla": 12.9,
    "VSStr": 9.5,
    "Vita": 18.0,
    "Wgt": 9.2
},{
    "Fir": 10.9,
    "Foc": 21.0,
    "Hol": 8.8,
    "Imm": 25.0,
    "Lit": 7.1,
    "Mag": 8.8,
    "Name": "Eye Surcoat",
    "Phy": 12.9,
    "Poi": 15.0,
    "PoiPerWgt": 1.6304347826086958,
    "Robu": 55.0,
    "VSPie": 11.9,
    "VSSla": 12.9,
    "VSStr": 8.8,
    "Vita": 18.0,
    "Wgt": 9.2
},{
    "Fir": 12.4,
    "Foc": 76.0,
    "Hol": 13.0,
    "Imm": 46.0,
    "Lit": 12.6,
    "Mag": 12.6,
    "Name": "Fia's Robe",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 21.0,
    "VSPie": 5.3,
    "VSSla": 5.3,
    "VSStr": 6.1,
    "Vita": 108.0,
    "Wgt": 5.1
},{
    "Fir": 11.4,
    "Foc": 57.0,
    "Hol": 12.6,
    "Imm": 34.0,
    "Lit": 11.9,
    "Mag": 11.9,
    "Name": "Fia's Robe (Altered)",
    "Phy": 2.7,
    "Poi": 0.0,
    "PoiPerWgt": 0.0,
    "Robu": 10.0,
    "VSPie": 2.7,
    "VSSla": 2.7,
    "VSStr": 4.2,
    "Vita": 93.0,
    "Wgt": 3.2
},{
    "Fir": 12.4,
    "Foc": 71.0,
    "Hol": 12.8,
    "Imm": 42.0,
    "Lit": 12.6,
    "Mag": 13.0,
    "Name": "Fell Omen Cloak",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 25.0,
    "VSPie": 6.1,
    "VSSla": 6.7,
    "VSStr": 6.7,
    "Vita": 83.0,
    "Wgt": 5.1
},{
    "Fir": 13.1,
    "Foc": 63.0,
    "Hol": 13.8,
    "Imm": 38.0,
    "Lit": 13.3,
    "Mag": 13.8,
    "Name": "Finger Maiden Robe",
    "Phy": 5.8,
    "Poi": 2.0,
    "PoiPerWgt": 0.4347826086956522,
    "Robu": 21.0,
    "VSPie": 3.2,
    "VSSla": 4.7,
    "VSStr": 4.7,
    "Vita": 63.0,
    "Wgt": 4.6
},{
    "Fir": 13.3,
    "Foc": 24.0,
    "Hol": 9.5,
    "Imm": 32.0,
    "Lit": 9.5,
    "Mag": 10.9,
    "Name": "Fire Monk Armor",
    "Phy": 14.0,
    "Poi": 16.0,
    "PoiPerWgt": 1.3559322033898304,
    "Robu": 55.0,
    "VSPie": 12.4,
    "VSSla": 13.5,
    "VSStr": 12.4,
    "Vita": 24.0,
    "Wgt": 11.8
},{
    "Fir": 7.1,
    "Foc": 55.0,
    "Hol": 7.1,
    "Imm": 63.0,
    "Lit": 8.8,
    "Mag": 6.7,
    "Name": "Fur Raiment",
    "Phy": 6.1,
    "Poi": 1.0,
    "PoiPerWgt": 0.19607843137254904,
    "Robu": 42.0,
    "VSPie": 6.1,
    "VSSla": 6.1,
    "VSStr": 6.7,
    "Vita": 38.0,
    "Wgt": 5.1
},{
    "Fir": 12.4,
    "Foc": 24.0,
    "Hol": 10.9,
    "Imm": 35.0,
    "Lit": 10.2,
    "Mag": 10.9,
    "Name": "Gelmir Knight Armor",
    "Phy": 13.5,
    "Poi": 19.0,
    "PoiPerWgt": 1.6101694915254237,
    "Robu": 55.0,
    "VSPie": 13.5,
    "VSSla": 14.0,
    "VSStr": 11.4,
    "Vita": 23.0,
    "Wgt": 11.8
},{
    "Fir": 11.9,
    "Foc": 24.0,
    "Hol": 10.9,
    "Imm": 35.0,
    "Lit": 9.5,
    "Mag": 10.9,
    "Name": "Godrick Knight Armor",
    "Phy": 13.5,
    "Poi": 19.0,
    "PoiPerWgt": 1.6101694915254237,
    "Robu": 57.0,
    "VSPie": 12.9,
    "VSSla": 14.0,
    "VSStr": 11.9,
    "Vita": 24.0,
    "Wgt": 11.8
},{
    "Fir": 11.9,
    "Foc": 67.0,
    "Hol": 13.8,
    "Imm": 42.0,
    "Lit": 12.4,
    "Mag": 12.6,
    "Name": "Godskin Apostle Robe",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 21.0,
    "VSPie": 2.7,
    "VSSla": 5.3,
    "VSStr": 5.3,
    "Vita": 63.0,
    "Wgt": 4.1
},{
    "Fir": 10.9,
    "Foc": 23.0,
    "Hol": 9.5,
    "Imm": 32.0,
    "Lit": 8.0,
    "Mag": 10.2,
    "Name": "Tree-and-Beast Surcoat",
    "Phy": 12.9,
    "Poi": 16.0,
    "PoiPerWgt": 1.509433962264151,
    "Robu": 55.0,
    "VSPie": 12.4,
    "VSSla": 12.9,
    "VSStr": 11.4,
    "Vita": 23.0,
    "Wgt": 10.6
},{
    "Fir": 10.2,
    "Foc": 23.0,
    "Hol": 10.2,
    "Imm": 32.0,
    "Lit": 8.0,
    "Mag": 9.5,
    "Name": "Haligtree Crest Surcoat",
    "Phy": 12.9,
    "Poi": 15.0,
    "PoiPerWgt": 1.4150943396226416,
    "Robu": 55.0,
    "VSPie": 12.9,
    "VSSla": 12.4,
    "VSStr": 11.9,
    "Vita": 24.0,
    "Wgt": 10.6
},{
    "Fir": 10.9,
    "Foc": 50.0,
    "Hol": 10.2,
    "Imm": 57.0,
    "Lit": 11.4,
    "Mag": 10.9,
    "Name": "Highwayman Cloth Armor",
    "Phy": 8.0,
    "Poi": 6.0,
    "PoiPerWgt": 0.7792207792207793,
    "Robu": 35.0,
    "VSPie": 8.8,
    "VSSla": 8.0,
    "VSStr": 10.2,
    "Vita": 50.0,
    "Wgt": 7.7
},{
    "Fir": 11.9,
    "Foc": 24.0,
    "Hol": 10.2,
    "Imm": 38.0,
    "Lit": 10.2,
    "Mag": 10.9,
    "Name": "Hoslow's Armor",
    "Phy": 14.0,
    "Poi": 16.0,
    "PoiPerWgt": 1.3559322033898304,
    "Robu": 60.0,
    "VSPie": 12.4,
    "VSSla": 13.5,
    "VSStr": 11.9,
    "Vita": 24.0,
    "Wgt": 11.8
},{
    "Fir": 11.9,
    "Foc": 63.0,
    "Hol": 12.6,
    "Imm": 38.0,
    "Lit": 12.4,
    "Mag": 12.8,
    "Name": "Juvenile Scholar Robe",
    "Phy": 5.3,
    "Poi": 1.0,
    "PoiPerWgt": 0.24390243902439027,
    "Robu": 25.0,
    "VSPie": 5.3,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 71.0,
    "Wgt": 4.1
},{
    "Fir": 10.9,
    "Foc": 18.0,
    "Hol": 8.8,
    "Imm": 28.0,
    "Lit": 9.5,
    "Mag": 10.9,
    "Name": "Knight Armor",
    "Phy": 12.4,
    "Poi": 19.0,
    "PoiPerWgt": 1.7924528301886793,
    "Robu": 50.0,
    "VSPie": 13.5,
    "VSSla": 13.5,
    "VSStr": 11.9,
    "Vita": 18.0,
    "Wgt": 10.6
},{
    "Fir": 8.8,
    "Foc": 11.0,
    "Hol": 8.0,
    "Imm": 25.0,
    "Lit": 7.1,
    "Mag": 8.0,
    "Name": "Kaiden Armor",
    "Phy": 11.9,
    "Poi": 14.0,
    "PoiPerWgt": 1.5909090909090908,
    "Robu": 55.0,
    "VSPie": 11.9,
    "VSSla": 11.9,
    "VSStr": 8.8,
    "Vita": 11.0,
    "Wgt": 8.8
},{
    "Fir": 11.4,
    "Foc": 50.0,
    "Hol": 10.9,
    "Imm": 60.0,
    "Lit": 11.9,
    "Mag": 10.2,
    "Name": "Land of Reeds Armor",
    "Phy": 8.8,
    "Poi": 6.0,
    "PoiPerWgt": 0.7228915662650601,
    "Robu": 50.0,
    "VSPie": 9.5,
    "VSSla": 11.9,
    "VSStr": 9.5,
    "Vita": 55.0,
    "Wgt": 8.3
},{
    "Fir": 10.9,
    "Foc": 55.0,
    "Hol": 10.9,
    "Imm": 63.0,
    "Lit": 12.4,
    "Mag": 10.2,
    "Name": "Leather-Draped Tabard",
    "Phy": 9.5,
    "Poi": 8.0,
    "PoiPerWgt": 0.9638554216867469,
    "Robu": 42.0,
    "VSPie": 10.2,
    "VSSla": 8.8,
    "VSStr": 10.9,
    "Vita": 55.0,
    "Wgt": 8.3
},{
    "Fir": 14.9,
    "Foc": 42.0,
    "Hol": 13.5,
    "Imm": 63.0,
    "Lit": 13.0,
    "Mag": 13.5,
    "Name": "Lionel's Armor",
    "Phy": 17.5,
    "Poi": 37.0,
    "PoiPerWgt": 1.7452830188679247,
    "Robu": 99.0,
    "VSPie": 19.7,
    "VSSla": 19.2,
    "VSStr": 16.3,
    "Vita": 50.0,
    "Wgt": 21.2
},{
    "Fir": 12.0,
    "Foc": 55.0,
    "Hol": 13.3,
    "Imm": 63.0,
    "Lit": 12.2,
    "Mag": 10.0,
    "Name": "Lord of Blood's Robe",
    "Phy": 9.3,
    "Poi": 8.0,
    "PoiPerWgt": 0.9876543209876544,
    "Robu": 23.0,
    "VSPie": 8.6,
    "VSSla": 8.6,
    "VSStr": 10.0,
    "Vita": 55.0,
    "Wgt": 8.1
},{
    "Fir": 13.0,
    "Foc": 38.0,
    "Hol": 13.0,
    "Imm": 57.0,
    "Lit": 13.8,
    "Mag": 13.0,
    "Name": "Malformed Dragon Armor",
    "Phy": 16.8,
    "Poi": 26.0,
    "PoiPerWgt": 1.625,
    "Robu": 76.0,
    "VSPie": 16.8,
    "VSSla": 17.5,
    "VSStr": 15.0,
    "Vita": 38.0,
    "Wgt": 16.0
},{
    "Fir": 11.4,
    "Foc": 24.0,
    "Hol": 12.8,
    "Imm": 35.0,
    "Lit": 9.5,
    "Mag": 10.9,
    "Name": "Maliketh's Armor",
    "Phy": 13.5,
    "Poi": 19.0,
    "PoiPerWgt": 1.484375,
    "Robu": 57.0,
    "VSPie": 13.5,
    "VSSla": 14.0,
    "VSStr": 11.9,
    "Vita": 50.0,
    "Wgt": 12.8
},{
    "Fir": 12.4,
    "Foc": 71.0,
    "Hol": 12.6,
    "Imm": 42.0,
    "Lit": 12.4,
    "Mag": 12.6,
    "Name": "Marais Robe",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 28.0,
    "VSPie": 2.7,
    "VSSla": 4.2,
    "VSStr": 6.1,
    "Vita": 67.0,
    "Wgt": 4.1
},{
    "Fir": 5.2,
    "Foc": 90.0,
    "Hol": 13.0,
    "Imm": 102.0,
    "Lit": 12.8,
    "Mag": 13.3,
    "Name": "Mushroom Body",
    "Phy": 6.1,
    "Poi": 1.0,
    "PoiPerWgt": 0.19607843137254904,
    "Robu": 24.0,
    "VSPie": 5.3,
    "VSSla": 2.7,
    "VSStr": 7.1,
    "Vita": 76.0,
    "Wgt": 5.1
},{
    "Fir": 13.5,
    "Foc": 76.0,
    "Hol": 13.3,
    "Imm": 46.0,
    "Lit": 13.0,
    "Mag": 13.3,
    "Name": "Noble's Traveling Garb",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.31746031746031744,
    "Robu": 24.0,
    "VSPie": 6.7,
    "VSSla": 6.1,
    "VSStr": 6.7,
    "Vita": 83.0,
    "Wgt": 6.3
},{
    "Fir": 10.2,
    "Foc": 35.0,
    "Hol": 7.1,
    "Imm": 42.0,
    "Lit": 9.5,
    "Mag": 9.5,
    "Name": "Old Aristocrat Gown",
    "Phy": 7.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 25.0,
    "VSPie": 7.1,
    "VSSla": 6.7,
    "VSStr": 7.1,
    "Vita": 38.0,
    "Wgt": 5.1
},{
    "Fir": 9.5,
    "Foc": 46.0,
    "Hol": 10.2,
    "Imm": 67.0,
    "Lit": 11.4,
    "Mag": 8.8,
    "Name": "Omenkiller Robe",
    "Phy": 8.8,
    "Poi": 6.0,
    "PoiPerWgt": 0.8450704225352114,
    "Robu": 28.0,
    "VSPie": 7.1,
    "VSSla": 7.1,
    "VSStr": 8.0,
    "Vita": 46.0,
    "Wgt": 7.1
},{
    "Fir": 13.0,
    "Foc": 76.0,
    "Hol": 13.0,
    "Imm": 57.0,
    "Lit": 12.6,
    "Mag": 13.0,
    "Name": "Perfumer's Traveling Garb",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 23.0,
    "VSPie": 4.2,
    "VSSla": 5.3,
    "VSStr": 6.1,
    "Vita": 71.0,
    "Wgt": 5.1
},{
    "Fir": 13.3,
    "Foc": 83.0,
    "Hol": 12.8,
    "Imm": 50.0,
    "Lit": 12.8,
    "Mag": 14.5,
    "Name": "Preceptor's Long Gown",
    "Phy": 6.1,
    "Poi": 3.0,
    "PoiPerWgt": 0.4761904761904762,
    "Robu": 24.0,
    "VSPie": 6.1,
    "VSSla": 6.1,
    "VSStr": 6.7,
    "Vita": 76.0,
    "Wgt": 6.3
},{
    "Fir": 11.4,
    "Foc": 63.0,
    "Hol": 12.6,
    "Imm": 42.0,
    "Lit": 11.4,
    "Mag": 11.9,
    "Name": "Prisoner Clothing",
    "Phy": 4.2,
    "Poi": 1.0,
    "PoiPerWgt": 0.3125,
    "Robu": 23.0,
    "VSPie": 4.2,
    "VSSla": 5.3,
    "VSStr": 5.3,
    "Vita": 63.0,
    "Wgt": 3.2
},{
    "Fir": 12.4,
    "Foc": 76.0,
    "Hol": 13.0,
    "Imm": 46.0,
    "Lit": 12.6,
    "Mag": 13.5,
    "Name": "Prophet Robe",
    "Phy": 6.7,
    "Poi": 2.0,
    "PoiPerWgt": 0.31746031746031744,
    "Robu": 25.0,
    "VSPie": 6.7,
    "VSSla": 6.1,
    "VSStr": 6.7,
    "Vita": 83.0,
    "Wgt": 6.3
},{
    "Fir": 12.6,
    "Foc": 71.0,
    "Hol": 13.3,
    "Imm": 42.0,
    "Lit": 12.8,
    "Mag": 13.8,
    "Name": "Queen's Robe",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 25.0,
    "VSPie": 4.2,
    "VSSla": 5.3,
    "VSStr": 5.3,
    "Vita": 83.0,
    "Wgt": 5.1
},{
    "Fir": 14.1,
    "Foc": 42.0,
    "Hol": 13.5,
    "Imm": 60.0,
    "Lit": 12.6,
    "Mag": 13.5,
    "Name": "Radahn's Lion Armor",
    "Phy": 18.7,
    "Poi": 28.0,
    "PoiPerWgt": 1.6,
    "Robu": 99.0,
    "VSPie": 17.5,
    "VSSla": 18.3,
    "VSStr": 15.0,
    "Vita": 38.0,
    "Wgt": 17.5
},{
    "Fir": 11.4,
    "Foc": 18.0,
    "Hol": 9.5,
    "Imm": 32.0,
    "Lit": 7.1,
    "Mag": 10.2,
    "Name": "Raging Wolf Armor",
    "Phy": 13.2,
    "Poi": 15.0,
    "PoiPerWgt": 1.4851485148514851,
    "Robu": 57.0,
    "VSPie": 12.9,
    "VSSla": 13.5,
    "VSStr": 11.6,
    "Vita": 18.0,
    "Wgt": 10.1
},{
    "Fir": 12.6,
    "Foc": 67.0,
    "Hol": 12.8,
    "Imm": 42.0,
    "Lit": 12.4,
    "Mag": 13.5,
    "Name": "Raya Lucarian Robe",
    "Phy": 5.3,
    "Poi": 1.0,
    "PoiPerWgt": 0.24390243902439027,
    "Robu": 18.0,
    "VSPie": 2.7,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 63.0,
    "Wgt": 4.1
},{
    "Fir": 8.8,
    "Foc": 42.0,
    "Hol": 8.8,
    "Imm": 76.0,
    "Lit": 10.9,
    "Mag": 8.8,
    "Name": "Rotten Gravekeeper Cloak",
    "Phy": 6.7,
    "Poi": 2.0,
    "PoiPerWgt": 0.31746031746031744,
    "Robu": 32.0,
    "VSPie": 6.7,
    "VSSla": 6.7,
    "VSStr": 6.7,
    "Vita": 42.0,
    "Wgt": 6.3
},{
    "Fir": 12.4,
    "Foc": 76.0,
    "Hol": 13.3,
    "Imm": 46.0,
    "Lit": 12.8,
    "Mag": 13.0,
    "Name": "Ruler's Robe",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 24.0,
    "VSPie": 4.2,
    "VSSla": 6.7,
    "VSStr": 6.7,
    "Vita": 76.0,
    "Wgt": 5.1
},{
    "Fir": 12.6,
    "Foc": 71.0,
    "Hol": 13.5,
    "Imm": 38.0,
    "Lit": 13.0,
    "Mag": 13.5,
    "Name": "Sage Robe",
    "Phy": 6.7,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 23.0,
    "VSPie": 4.2,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 76.0,
    "Wgt": 5.1
},{
    "Fir": 11.9,
    "Foc": 83.0,
    "Hol": 13.5,
    "Imm": 50.0,
    "Lit": 13.0,
    "Mag": 13.3,
    "Name": "Sanguine Noble Robe",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 21.0,
    "VSPie": 5.3,
    "VSSla": 5.3,
    "VSStr": 5.3,
    "Vita": 76.0,
    "Wgt": 5.1
},{
    "Fir": 10.9,
    "Foc": 11.0,
    "Hol": 8.0,
    "Imm": 25.0,
    "Lit": 6.7,
    "Mag": 7.1,
    "Name": "Scale Armor",
    "Phy": 11.9,
    "Poi": 14.0,
    "PoiPerWgt": 1.5909090909090908,
    "Robu": 46.0,
    "VSPie": 10.9,
    "VSSla": 12.4,
    "VSStr": 10.2,
    "Vita": 21.0,
    "Wgt": 8.8
},{
    "Fir": 14.1,
    "Foc": 37.0,
    "Hol": 13.5,
    "Imm": 57.0,
    "Lit": 13.0,
    "Mag": 13.5,
    "Name": "Scaled Armor",
    "Phy": 16.0,
    "Poi": 27.0,
    "PoiPerWgt": 1.6875,
    "Robu": 83.0,
    "VSPie": 16.0,
    "VSSla": 16.8,
    "VSStr": 13.9,
    "Vita": 38.0,
    "Wgt": 16.0
},{
    "Fir": 13.0,
    "Foc": 71.0,
    "Hol": 13.0,
    "Imm": 38.0,
    "Lit": 12.8,
    "Mag": 13.0,
    "Name": "Snow Witch Robe",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.36363636363636365,
    "Robu": 32.0,
    "VSPie": 4.2,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 71.0,
    "Wgt": 5.5
},{
    "Fir": 12.9,
    "Foc": 76.0,
    "Hol": 13.4,
    "Imm": 46.0,
    "Lit": 13.1,
    "Mag": 13.4,
    "Name": "Traveling Maiden Robe",
    "Phy": 5.4,
    "Poi": 2.0,
    "PoiPerWgt": 0.4081632653061224,
    "Robu": 24.0,
    "VSPie": 4.3,
    "VSSla": 6.2,
    "VSStr": 6.2,
    "Vita": 83.0,
    "Wgt": 4.9
},{
    "Fir": 12.4,
    "Foc": 24.0,
    "Hol": 11.4,
    "Imm": 38.0,
    "Lit": 10.2,
    "Mag": 12.4,
    "Name": "Twinned Armor",
    "Phy": 14.6,
    "Poi": 21.0,
    "PoiPerWgt": 1.5328467153284673,
    "Robu": 60.0,
    "VSPie": 13.5,
    "VSSla": 16.0,
    "VSStr": 13.3,
    "Vita": 60.0,
    "Wgt": 13.7
},{
    "Fir": 11.9,
    "Foc": 67.0,
    "Hol": 13.0,
    "Imm": 42.0,
    "Lit": 12.6,
    "Mag": 12.8,
    "Name": "Upper-Class Robe",
    "Phy": 4.2,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 23.0,
    "VSPie": 2.7,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 67.0,
    "Wgt": 4.1
},{
    "Fir": 10.9,
    "Foc": 23.0,
    "Hol": 8.8,
    "Imm": 35.0,
    "Lit": 8.8,
    "Mag": 10.2,
    "Name": "Vagabond Knight Armor",
    "Phy": 13.5,
    "Poi": 15.0,
    "PoiPerWgt": 1.4150943396226416,
    "Robu": 57.0,
    "VSPie": 11.9,
    "VSSla": 12.4,
    "VSStr": 11.4,
    "Vita": 23.0,
    "Wgt": 10.6
},{
    "Fir": 14.1,
    "Foc": 46.0,
    "Hol": 13.3,
    "Imm": 63.0,
    "Lit": 12.8,
    "Mag": 13.5,
    "Name": "Veteran's Armor",
    "Phy": 18.7,
    "Poi": 37.0,
    "PoiPerWgt": 1.957671957671958,
    "Robu": 90.0,
    "VSPie": 17.5,
    "VSSla": 18.7,
    "VSStr": 17.0,
    "Vita": 46.0,
    "Wgt": 18.9
},{
    "Fir": 10.9,
    "Foc": 55.0,
    "Hol": 10.2,
    "Imm": 67.0,
    "Lit": 11.9,
    "Mag": 10.9,
    "Name": "White Reed Armor",
    "Phy": 9.5,
    "Poi": 8.0,
    "PoiPerWgt": 0.9638554216867469,
    "Robu": 42.0,
    "VSPie": 8.8,
    "VSSla": 11.4,
    "VSStr": 10.2,
    "Vita": 50.0,
    "Wgt": 8.3
},{
    "Fir": 8.8,
    "Foc": 11.0,
    "Hol": 8.0,
    "Imm": 21.0,
    "Lit": 6.1,
    "Mag": 8.8,
    "Name": "Zamor Armor",
    "Phy": 11.9,
    "Poi": 12.0,
    "PoiPerWgt": 1.3636363636363635,
    "Robu": 63.0,
    "VSPie": 10.9,
    "VSSla": 12.4,
    "VSStr": 9.5,
    "Vita": 18.0,
    "Wgt": 8.8
},{
    "Fir": 8.8,
    "Foc": 21.0,
    "Hol": 8.8,
    "Imm": 24.0,
    "Lit": 6.7,
    "Mag": 8.8,
    "Name": "Marionette Soldier Armor",
    "Phy": 11.9,
    "Poi": 14.0,
    "PoiPerWgt": 1.5909090909090908,
    "Robu": 50.0,
    "VSPie": 8.8,
    "VSSla": 13.5,
    "VSStr": 8.8,
    "Vita": 18.0,
    "Wgt": 8.8
},{
    "Fir": 11.4,
    "Foc": 57.0,
    "Hol": 12.6,
    "Imm": 32.0,
    "Lit": 11.9,
    "Mag": 12.6,
    "Name": "Alberich's Robe (Altered)",
    "Phy": 4.2,
    "Poi": 0.0,
    "PoiPerWgt": 0.0,
    "Robu": 19.0,
    "VSPie": 4.2,
    "VSSla": 4.2,
    "VSStr": 2.7,
    "Vita": 61.0,
    "Wgt": 3.2
},{
    "Fir": 8.8,
    "Foc": 16.0,
    "Hol": 8.0,
    "Imm": 22.0,
    "Lit": 9.5,
    "Mag": 11.4,
    "Name": "All-Knowing Armor (Altered)",
    "Phy": 12.4,
    "Poi": 13.0,
    "PoiPerWgt": 1.3978494623655913,
    "Robu": 38.0,
    "VSPie": 11.4,
    "VSSla": 12.9,
    "VSStr": 10.9,
    "Vita": 16.0,
    "Wgt": 9.3
},{
    "Fir": 6.7,
    "Foc": 32.0,
    "Hol": 6.1,
    "Imm": 42.0,
    "Lit": 7.1,
    "Mag": 6.1,
    "Name": "Aristocrat Garb (Altered)",
    "Phy": 7.1,
    "Poi": 1.0,
    "PoiPerWgt": 0.24390243902439027,
    "Robu": 29.0,
    "VSPie": 7.1,
    "VSSla": 7.1,
    "VSStr": 8.0,
    "Vita": 25.0,
    "Wgt": 4.1
},{
    "Fir": 12.8,
    "Foc": 75.0,
    "Hol": 12.8,
    "Imm": 42.0,
    "Lit": 13.0,
    "Mag": 13.3,
    "Name": "Astrologer Robe (Altered)",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.37735849056603776,
    "Robu": 21.0,
    "VSPie": 5.3,
    "VSSla": 5.3,
    "VSStr": 5.3,
    "Vita": 64.0,
    "Wgt": 5.3
},{
    "Fir": 13.3,
    "Foc": 32.0,
    "Hol": 13.0,
    "Imm": 52.0,
    "Lit": 12.8,
    "Mag": 13.3,
    "Name": "Banished Knight Armor (Altered)",
    "Phy": 18.3,
    "Poi": 25.0,
    "PoiPerWgt": 1.5151515151515151,
    "Robu": 69.0,
    "VSPie": 16.8,
    "VSSla": 18.7,
    "VSStr": 14.4,
    "Vita": 34.0,
    "Wgt": 16.5
},{
    "Fir": 11.9,
    "Foc": 63.0,
    "Hol": 12.6,
    "Imm": 42.0,
    "Lit": 12.4,
    "Mag": 13.0,
    "Name": "Battlemage Robe",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 23.0,
    "VSPie": 5.3,
    "VSSla": 4.2,
    "VSStr": 6.1,
    "Vita": 71.0,
    "Wgt": 4.1
},{
    "Fir": 13.7,
    "Foc": 34.0,
    "Hol": 13.4,
    "Imm": 50.0,
    "Lit": 12.7,
    "Mag": 12.9,
    "Name": "Beast Champion Armor (Altered)",
    "Phy": 17.2,
    "Poi": 25.0,
    "PoiPerWgt": 1.524390243902439,
    "Robu": 75.0,
    "VSPie": 18.2,
    "VSSla": 18.5,
    "VSStr": 15.9,
    "Vita": 32.0,
    "Wgt": 16.4
},{
    "Fir": 9.4,
    "Foc": 21.0,
    "Hol": 11.3,
    "Imm": 25.0,
    "Lit": 6.6,
    "Mag": 8.7,
    "Name": "Black Knife Armor (Altered)",
    "Phy": 11.3,
    "Poi": 12.0,
    "PoiPerWgt": 1.3333333333333333,
    "Robu": 42.0,
    "VSPie": 12.3,
    "VSSla": 12.3,
    "VSStr": 10.8,
    "Vita": 21.0,
    "Wgt": 9.0
},{
    "Fir": 12.6,
    "Foc": 25.0,
    "Hol": 11.9,
    "Imm": 38.0,
    "Lit": 10.2,
    "Mag": 11.4,
    "Name": "Blaidd's Armor",
    "Phy": 14.6,
    "Poi": 24.0,
    "PoiPerWgt": 1.7518248175182483,
    "Robu": 76.0,
    "VSPie": 15.3,
    "VSSla": 14.6,
    "VSStr": 12.9,
    "Vita": 25.0,
    "Wgt": 13.7
},{
    "Fir": 12.4,
    "Foc": 22.0,
    "Hol": 11.4,
    "Imm": 32.0,
    "Lit": 9.5,
    "Mag": 10.9,
    "Name": "Blaidd's Armor (Altered)",
    "Phy": 14.0,
    "Poi": 20.0,
    "PoiPerWgt": 1.6,
    "Robu": 57.0,
    "VSPie": 14.6,
    "VSSla": 14.0,
    "VSStr": 12.4,
    "Vita": 21.0,
    "Wgt": 12.5
},{
    "Fir": 9.9,
    "Foc": 21.0,
    "Hol": 9.9,
    "Imm": 29.0,
    "Lit": 6.8,
    "Mag": 9.2,
    "Name": "Bloodhound Knight Armor (Altered)",
    "Phy": 12.1,
    "Poi": 12.0,
    "PoiPerWgt": 1.25,
    "Robu": 45.0,
    "VSPie": 13.2,
    "VSSla": 13.7,
    "VSStr": 11.1,
    "Vita": 21.0,
    "Wgt": 9.6
},{
    "Fir": 10.9,
    "Foc": 55.0,
    "Hol": 11.4,
    "Imm": 63.0,
    "Lit": 12.4,
    "Mag": 10.9,
    "Name": "Bloodsoaked Tabard",
    "Phy": 9.5,
    "Poi": 9.0,
    "PoiPerWgt": 1.0843373493975903,
    "Robu": 38.0,
    "VSPie": 9.5,
    "VSSla": 10.2,
    "VSStr": 9.5,
    "Vita": 50.0,
    "Wgt": 8.3
},{
    "Fir": 11.4,
    "Foc": 71.0,
    "Hol": 13.8,
    "Imm": 46.0,
    "Lit": 12.6,
    "Mag": 12.6,
    "Name": "Blue Festive Garb",
    "Phy": 4.2,
    "Poi": 1.0,
    "PoiPerWgt": 0.24390243902439027,
    "Robu": 21.0,
    "VSPie": 5.3,
    "VSSla": 6.1,
    "VSStr": 4.2,
    "Vita": 63.0,
    "Wgt": 4.1
},{
    "Fir": 9.7,
    "Foc": 10.0,
    "Hol": 7.3,
    "Imm": 21.0,
    "Lit": 7.3,
    "Mag": 10.4,
    "Name": "Blue Silver Mail Armor (Altered)",
    "Phy": 12.1,
    "Poi": 10.0,
    "PoiPerWgt": 1.1764705882352942,
    "Robu": 52.0,
    "VSPie": 11.1,
    "VSSla": 12.6,
    "VSStr": 9.0,
    "Vita": 10.0,
    "Wgt": 8.5
},{
    "Fir": 12.4,
    "Foc": 67.0,
    "Hol": 12.8,
    "Imm": 42.0,
    "Lit": 12.6,
    "Mag": 12.8,
    "Name": "Brave's Battlewear",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 23.0,
    "VSPie": 4.2,
    "VSSla": 5.3,
    "VSStr": 5.3,
    "Vita": 67.0,
    "Wgt": 4.1
},{
    "Fir": 11.9,
    "Foc": 57.0,
    "Hol": 12.6,
    "Imm": 34.0,
    "Lit": 12.4,
    "Mag": 12.6,
    "Name": "Brave's Battlewear (Altered)",
    "Phy": 4.2,
    "Poi": 1.0,
    "PoiPerWgt": 0.3125,
    "Robu": 19.0,
    "VSPie": 2.7,
    "VSSla": 4.2,
    "VSStr": 4.2,
    "Vita": 57.0,
    "Wgt": 3.2
},{
    "Fir": 11.4,
    "Foc": 21.0,
    "Hol": 9.5,
    "Imm": 29.0,
    "Lit": 7.1,
    "Mag": 10.2,
    "Name": "Briar Armor (Altered)",
    "Phy": 12.4,
    "Poi": 12.0,
    "PoiPerWgt": 1.1214953271028039,
    "Robu": 61.0,
    "VSPie": 11.9,
    "VSSla": 12.4,
    "VSStr": 10.9,
    "Vita": 21.0,
    "Wgt": 10.7
},{
    "Fir": 11.7,
    "Foc": 16.0,
    "Hol": 11.7,
    "Imm": 22.0,
    "Lit": 9.1,
    "Mag": 11.7,
    "Name": "Carian Knight Armor (Altered)",
    "Phy": 11.7,
    "Poi": 8.0,
    "PoiPerWgt": 0.8247422680412372,
    "Robu": 38.0,
    "VSPie": 11.7,
    "VSSla": 12.2,
    "VSStr": 9.8,
    "Vita": 19.0,
    "Wgt": 9.7
},{
    "Fir": 12.4,
    "Foc": 22.0,
    "Hol": 13.0,
    "Imm": 52.0,
    "Lit": 10.2,
    "Mag": 11.9,
    "Name": "Cleanrot Armor (Altered)",
    "Phy": 13.5,
    "Poi": 17.0,
    "PoiPerWgt": 1.2408759124087592,
    "Robu": 54.0,
    "VSPie": 16.0,
    "VSSla": 14.6,
    "VSStr": 12.4,
    "Vita": 22.0,
    "Wgt": 13.7
},{
    "Fir": 11.9,
    "Foc": 61.0,
    "Hol": 12.6,
    "Imm": 34.0,
    "Lit": 12.4,
    "Mag": 12.6,
    "Name": "Commoner's Garb (Altered)",
    "Phy": 4.2,
    "Poi": 0.0,
    "PoiPerWgt": 0.0,
    "Robu": 21.0,
    "VSPie": 5.3,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 64.0,
    "Wgt": 4.1
},{
    "Fir": 12.4,
    "Foc": 61.0,
    "Hol": 12.6,
    "Imm": 38.0,
    "Lit": 12.4,
    "Mag": 12.8,
    "Name": "Commoner's Simple Garb (Altered)",
    "Phy": 3.2,
    "Poi": 0.0,
    "PoiPerWgt": 0.0,
    "Robu": 21.0,
    "VSPie": 5.3,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 64.0,
    "Wgt": 4.1
},{
    "Fir": 10.5,
    "Foc": 42.0,
    "Hol": 9.1,
    "Imm": 54.0,
    "Lit": 11.5,
    "Mag": 10.5,
    "Name": "Confessor Armor (Altered)",
    "Phy": 7.6,
    "Poi": 4.0,
    "PoiPerWgt": 0.547945205479452,
    "Robu": 42.0,
    "VSPie": 8.4,
    "VSSla": 7.6,
    "VSStr": 9.8,
    "Vita": 45.0,
    "Wgt": 7.3
},{
    "Fir": 12.4,
    "Foc": 67.0,
    "Hol": 12.8,
    "Imm": 46.0,
    "Lit": 12.4,
    "Mag": 13.0,
    "Name": "Consort's Robe",
    "Phy": 5.3,
    "Poi": 1.0,
    "PoiPerWgt": 0.24390243902439027,
    "Robu": 25.0,
    "VSPie": 5.3,
    "VSSla": 4.2,
    "VSStr": 4.2,
    "Vita": 63.0,
    "Wgt": 4.1
},{
    "Fir": 12.8,
    "Foc": 107.0,
    "Hol": 13.5,
    "Imm": 46.0,
    "Lit": 12.8,
    "Mag": 13.3,
    "Name": "Corhyn's Robe",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.31746031746031744,
    "Robu": 24.0,
    "VSPie": 6.1,
    "VSSla": 6.7,
    "VSStr": 6.7,
    "Vita": 76.0,
    "Wgt": 6.3
},{
    "Fir": 12.6,
    "Foc": 29.0,
    "Hol": 13.3,
    "Imm": 42.0,
    "Lit": 10.9,
    "Mag": 12.8,
    "Name": "Crucible Axe Armor (Altered)",
    "Phy": 16.8,
    "Poi": 23.0,
    "PoiPerWgt": 1.5436241610738255,
    "Robu": 61.0,
    "VSPie": 16.0,
    "VSSla": 16.0,
    "VSStr": 13.4,
    "Vita": 29.0,
    "Wgt": 14.9
},{
    "Fir": 11.4,
    "Foc": 29.0,
    "Hol": 14.1,
    "Imm": 42.0,
    "Lit": 10.9,
    "Mag": 12.8,
    "Name": "Crucible Tree Armor (Altered)",
    "Phy": 17.0,
    "Poi": 23.0,
    "PoiPerWgt": 1.5436241610738255,
    "Robu": 61.0,
    "VSPie": 15.6,
    "VSSla": 16.0,
    "VSStr": 13.4,
    "Vita": 29.0,
    "Wgt": 14.9
},{
    "Fir": 11.9,
    "Foc": 24.0,
    "Hol": 10.2,
    "Imm": 32.0,
    "Lit": 8.8,
    "Mag": 12.4,
    "Name": "Cuckoo Knight Armor",
    "Phy": 13.5,
    "Poi": 19.0,
    "PoiPerWgt": 1.6101694915254237,
    "Robu": 57.0,
    "VSPie": 13.5,
    "VSSla": 14.6,
    "VSStr": 11.4,
    "Vita": 24.0,
    "Wgt": 11.8
},{
    "Fir": 11.5,
    "Foc": 21.0,
    "Hol": 9.8,
    "Imm": 29.0,
    "Lit": 8.4,
    "Mag": 12.0,
    "Name": "Cuckoo Knight Armor (Altered)",
    "Phy": 13.1,
    "Poi": 17.0,
    "PoiPerWgt": 1.574074074074074,
    "Robu": 52.0,
    "VSPie": 13.1,
    "VSSla": 14.2,
    "VSStr": 11.0,
    "Vita": 21.0,
    "Wgt": 10.8
},{
    "Fir": 12.6,
    "Foc": 48.0,
    "Hol": 12.6,
    "Imm": 63.0,
    "Lit": 12.4,
    "Mag": 13.0,
    "Name": "Depraved Perfumer Robe (Altered)",
    "Phy": 5.3,
    "Poi": 1.0,
    "PoiPerWgt": 0.25,
    "Robu": 18.0,
    "VSPie": 2.6,
    "VSSla": 4.1,
    "VSStr": 5.2,
    "Vita": 63.0,
    "Wgt": 4.0
},{
    "Fir": 8.8,
    "Foc": 21.0,
    "Hol": 8.0,
    "Imm": 25.0,
    "Lit": 6.1,
    "Mag": 8.0,
    "Name": "Dirty Chainmail",
    "Phy": 11.4,
    "Poi": 14.0,
    "PoiPerWgt": 1.5909090909090908,
    "Robu": 50.0,
    "VSPie": 11.4,
    "VSSla": 11.9,
    "VSStr": 8.8,
    "Vita": 21.0,
    "Wgt": 8.8
},{
    "Fir": 11.4,
    "Foc": 21.0,
    "Hol": 10.2,
    "Imm": 25.0,
    "Lit": 8.8,
    "Mag": 10.2,
    "Name": "Drake Knight Armor",
    "Phy": 11.4,
    "Poi": 14.0,
    "PoiPerWgt": 1.5217391304347827,
    "Robu": 46.0,
    "VSPie": 11.4,
    "VSSla": 11.9,
    "VSStr": 9.5,
    "Vita": 21.0,
    "Wgt": 9.2
},{
    "Fir": 9.5,
    "Foc": 10.0,
    "Hol": 8.0,
    "Imm": 21.0,
    "Lit": 6.7,
    "Mag": 8.0,
    "Name": "Drake Knight Armor (Altered)",
    "Phy": 11.2,
    "Poi": 8.0,
    "PoiPerWgt": 0.9638554216867469,
    "Robu": 34.0,
    "VSPie": 10.7,
    "VSSla": 11.2,
    "VSStr": 8.6,
    "Vita": 10.0,
    "Wgt": 8.3
},{
    "Fir": 10.6,
    "Foc": 10.0,
    "Hol": 6.8,
    "Imm": 22.0,
    "Lit": 6.4,
    "Mag": 7.7,
    "Name": "Elden Lord Armor (Altered)",
    "Phy": 11.1,
    "Poi": 10.0,
    "PoiPerWgt": 1.1904761904761905,
    "Robu": 45.0,
    "VSPie": 11.6,
    "VSSla": 10.6,
    "VSStr": 9.9,
    "Vita": 16.0,
    "Wgt": 8.4
},{
    "Fir": 12.4,
    "Foc": 62.0,
    "Hol": 12.8,
    "Imm": 31.0,
    "Lit": 11.9,
    "Mag": 12.8,
    "Name": "Errant Sorcerer Robe (Altered)",
    "Phy": 2.7,
    "Poi": 1.0,
    "PoiPerWgt": 0.3125,
    "Robu": 20.0,
    "VSPie": 2.7,
    "VSSla": 5.3,
    "VSStr": 2.7,
    "Vita": 66.0,
    "Wgt": 3.2
},{
    "Fir": 11.9,
    "Foc": 67.0,
    "Hol": 13.0,
    "Imm": 42.0,
    "Lit": 12.4,
    "Mag": 12.6,
    "Name": "Festive Garb",
    "Phy": 4.2,
    "Poi": 1.0,
    "PoiPerWgt": 0.24390243902439027,
    "Robu": 21.0,
    "VSPie": 5.3,
    "VSSla": 6.1,
    "VSStr": 5.3,
    "Vita": 67.0,
    "Wgt": 4.1
},{
    "Fir": 11.4,
    "Foc": 57.0,
    "Hol": 12.8,
    "Imm": 34.0,
    "Lit": 11.9,
    "Mag": 12.4,
    "Name": "Festive Garb (Altered)",
    "Phy": 2.7,
    "Poi": 0.0,
    "PoiPerWgt": 0.0,
    "Robu": 16.0,
    "VSPie": 4.2,
    "VSSla": 5.3,
    "VSStr": 4.2,
    "Vita": 57.0,
    "Wgt": 3.2
},{
    "Fir": 12.4,
    "Foc": 54.0,
    "Hol": 13.5,
    "Imm": 32.0,
    "Lit": 12.6,
    "Mag": 13.0,
    "Name": "Finger Maiden Robe (Altered)",
    "Phy": 4.2,
    "Poi": 1.0,
    "PoiPerWgt": 0.27027027027027023,
    "Robu": 16.0,
    "VSPie": 0.6,
    "VSSla": 2.7,
    "VSStr": 2.7,
    "Vita": 54.0,
    "Wgt": 3.7
},{
    "Fir": 12.6,
    "Foc": 11.0,
    "Hol": 9.5,
    "Imm": 32.0,
    "Lit": 6.7,
    "Mag": 9.5,
    "Name": "Fingerprint Armor",
    "Phy": 13.5,
    "Poi": 19.0,
    "PoiPerWgt": 1.7924528301886793,
    "Robu": 60.0,
    "VSPie": 11.9,
    "VSSla": 11.9,
    "VSStr": 11.9,
    "Vita": 23.0,
    "Wgt": 10.6
},{
    "Fir": 12.2,
    "Foc": 10.0,
    "Hol": 9.1,
    "Imm": 29.0,
    "Lit": 6.3,
    "Mag": 9.1,
    "Name": "Fingerprint Armor (Altered)",
    "Phy": 13.1,
    "Poi": 17.0,
    "PoiPerWgt": 1.7,
    "Robu": 54.0,
    "VSPie": 11.5,
    "VSSla": 11.5,
    "VSStr": 11.5,
    "Vita": 21.0,
    "Wgt": 10.0
},{
    "Fir": 19.8,
    "Foc": 90.0,
    "Hol": 13.3,
    "Imm": 67.0,
    "Lit": 13.0,
    "Mag": 13.5,
    "Name": "Fire Prelate Armor",
    "Phy": 19.2,
    "Poi": 40.0,
    "PoiPerWgt": 1.6194331983805668,
    "Robu": 63.0,
    "VSPie": 18.3,
    "VSSla": 18.3,
    "VSStr": 17.0,
    "Vita": 50.0,
    "Wgt": 24.7
},{
    "Fir": 18.3,
    "Foc": 75.0,
    "Hol": 13.0,
    "Imm": 57.0,
    "Lit": 12.8,
    "Mag": 13.3,
    "Name": "Fire Prelate Armor (Altered)",
    "Phy": 18.7,
    "Poi": 37.0,
    "PoiPerWgt": 1.5677966101694913,
    "Robu": 54.0,
    "VSPie": 17.5,
    "VSSla": 17.5,
    "VSStr": 16.3,
    "Vita": 42.0,
    "Wgt": 23.6
},{
    "Fir": 11.4,
    "Foc": 55.0,
    "Hol": 10.2,
    "Imm": 63.0,
    "Lit": 11.4,
    "Mag": 11.4,
    "Name": "Foot Soldier Tabard",
    "Phy": 9.5,
    "Poi": 8.0,
    "PoiPerWgt": 0.9638554216867469,
    "Robu": 42.0,
    "VSPie": 10.2,
    "VSSla": 10.2,
    "VSStr": 9.5,
    "Vita": 55.0,
    "Wgt": 8.3
},{
    "Fir": 12.0,
    "Foc": 21.0,
    "Hol": 10.5,
    "Imm": 32.0,
    "Lit": 9.8,
    "Mag": 10.5,
    "Name": "Gelmir Knight Armor (Altered)",
    "Phy": 13.1,
    "Poi": 17.0,
    "PoiPerWgt": 1.574074074074074,
    "Robu": 50.0,
    "VSPie": 13.1,
    "VSSla": 13.6,
    "VSStr": 11.0,
    "Vita": 21.0,
    "Wgt": 10.8
},{
    "Fir": 11.5,
    "Foc": 21.0,
    "Hol": 10.5,
    "Imm": 32.0,
    "Lit": 9.1,
    "Mag": 10.5,
    "Name": "Godrick Knight Armor (Altered)",
    "Phy": 13.1,
    "Poi": 17.0,
    "PoiPerWgt": 1.574074074074074,
    "Robu": 52.0,
    "VSPie": 12.5,
    "VSSla": 13.6,
    "VSStr": 11.5,
    "Vita": 21.0,
    "Wgt": 10.8
},{
    "Fir": 12.4,
    "Foc": 76.0,
    "Hol": 14.1,
    "Imm": 46.0,
    "Lit": 12.6,
    "Mag": 13.0,
    "Name": "Godskin Noble Robe",
    "Phy": 6.1,
    "Poi": 5.0,
    "PoiPerWgt": 0.7936507936507937,
    "Robu": 25.0,
    "VSPie": 6.1,
    "VSSla": 6.7,
    "VSStr": 9.5,
    "Vita": 83.0,
    "Wgt": 6.3
},{
    "Fir": 11.4,
    "Foc": 60.0,
    "Hol": 12.8,
    "Imm": 38.0,
    "Lit": 11.9,
    "Mag": 11.9,
    "Name": "Goldmask's Rags",
    "Phy": 4.2,
    "Poi": 1.0,
    "PoiPerWgt": 0.4166666666666667,
    "Robu": 11.0,
    "VSPie": 0.6,
    "VSSla": 2.7,
    "VSStr": 2.7,
    "Vita": 60.0,
    "Wgt": 2.4
},{
    "Fir": 9.5,
    "Foc": 34.0,
    "Hol": 8.8,
    "Imm": 63.0,
    "Lit": 10.2,
    "Mag": 8.0,
    "Name": "Gravekeeper Cloak",
    "Phy": 7.1,
    "Poi": 1.0,
    "PoiPerWgt": 0.15873015873015872,
    "Robu": 25.0,
    "VSPie": 7.1,
    "VSSla": 7.1,
    "VSStr": 8.8,
    "Vita": 42.0,
    "Wgt": 6.3
},{
    "Fir": 8.8,
    "Foc": 34.0,
    "Hol": 8.0,
    "Imm": 54.0,
    "Lit": 9.5,
    "Mag": 7.1,
    "Name": "Gravekeeper Cloak (Altered)",
    "Phy": 6.7,
    "Poi": 1.0,
    "PoiPerWgt": 0.19607843137254904,
    "Robu": 22.0,
    "VSPie": 6.7,
    "VSSla": 6.7,
    "VSStr": 8.0,
    "Vita": 34.0,
    "Wgt": 5.1
},{
    "Fir": 10.2,
    "Foc": 50.0,
    "Hol": 10.9,
    "Imm": 61.0,
    "Lit": 10.9,
    "Mag": 10.9,
    "Name": "Guardian Garb",
    "Phy": 9.5,
    "Poi": 4.0,
    "PoiPerWgt": 0.5194805194805194,
    "Robu": 38.0,
    "VSPie": 8.0,
    "VSSla": 8.8,
    "VSStr": 9.5,
    "Vita": 42.0,
    "Wgt": 7.7
},{
    "Fir": -2.3,
    "Foc": 57.0,
    "Hol": 11.4,
    "Imm": 71.0,
    "Lit": 11.9,
    "Mag": 11.9,
    "Name": "Guardian Garb (Full Bloom)",
    "Phy": 10.2,
    "Poi": 5.0,
    "PoiPerWgt": 0.5681818181818181,
    "Robu": 46.0,
    "VSPie": 9.5,
    "VSSla": 9.5,
    "VSStr": 10.2,
    "Vita": 55.0,
    "Wgt": 8.8
},{
    "Fir": 11.4,
    "Foc": 24.0,
    "Hol": 11.4,
    "Imm": 35.0,
    "Lit": 9.5,
    "Mag": 10.2,
    "Name": "Haligtree Knight Armor",
    "Phy": 13.5,
    "Poi": 16.0,
    "PoiPerWgt": 1.3559322033898304,
    "Robu": 57.0,
    "VSPie": 13.5,
    "VSSla": 13.5,
    "VSStr": 12.4,
    "Vita": 25.0,
    "Wgt": 11.8
},{
    "Fir": 11.0,
    "Foc": 21.0,
    "Hol": 11.0,
    "Imm": 32.0,
    "Lit": 9.1,
    "Mag": 9.8,
    "Name": "Haligtree Knight Armor (Altered)",
    "Phy": 13.1,
    "Poi": 14.0,
    "PoiPerWgt": 1.2962962962962963,
    "Robu": 52.0,
    "VSPie": 13.1,
    "VSSla": 13.1,
    "VSStr": 12.0,
    "Vita": 22.0,
    "Wgt": 10.8
},{
    "Fir": 12.8,
    "Foc": 76.0,
    "Hol": 13.2,
    "Imm": 42.0,
    "Lit": 13.0,
    "Mag": 13.5,
    "Name": "High Page Clothes",
    "Phy": 5.5,
    "Poi": 2.0,
    "PoiPerWgt": 0.37735849056603776,
    "Robu": 23.0,
    "VSPie": 5.5,
    "VSSla": 5.5,
    "VSStr": 6.9,
    "Vita": 76.0,
    "Wgt": 5.3
},{
    "Fir": 12.6,
    "Foc": 61.0,
    "Hol": 13.0,
    "Imm": 34.0,
    "Lit": 12.8,
    "Mag": 13.2,
    "Name": "High Page Clothes (Altered)",
    "Phy": 4.4,
    "Poi": 1.0,
    "PoiPerWgt": 0.23255813953488372,
    "Robu": 19.0,
    "VSPie": 4.4,
    "VSSla": 4.4,
    "VSStr": 6.3,
    "Vita": 61.0,
    "Wgt": 4.3
},{
    "Fir": 11.0,
    "Foc": 21.0,
    "Hol": 8.9,
    "Imm": 32.0,
    "Lit": 8.9,
    "Mag": 10.3,
    "Name": "Hoslow's Armor (Altered)",
    "Phy": 13.6,
    "Poi": 13.0,
    "PoiPerWgt": 1.2037037037037037,
    "Robu": 52.0,
    "VSPie": 12.0,
    "VSSla": 12.5,
    "VSStr": 11.5,
    "Vita": 21.0,
    "Wgt": 10.8
},{
    "Fir": 10.9,
    "Foc": 55.0,
    "Hol": 11.4,
    "Imm": 63.0,
    "Lit": 11.9,
    "Mag": 10.2,
    "Name": "Ivory-Draped Tabard",
    "Phy": 9.5,
    "Poi": 6.0,
    "PoiPerWgt": 0.7228915662650601,
    "Robu": 42.0,
    "VSPie": 10.2,
    "VSSla": 8.8,
    "VSStr": 10.9,
    "Vita": 57.0,
    "Wgt": 8.3
},{
    "Fir": 10.9,
    "Foc": 42.0,
    "Hol": 10.2,
    "Imm": 52.0,
    "Lit": 11.4,
    "Mag": 9.5,
    "Name": "Land of Reeds Armor (Altered)",
    "Phy": 8.0,
    "Poi": 4.0,
    "PoiPerWgt": 0.5128205128205129,
    "Robu": 42.0,
    "VSPie": 8.8,
    "VSSla": 11.4,
    "VSStr": 8.8,
    "Vita": 45.0,
    "Wgt": 7.8
},{
    "Fir": 11.9,
    "Foc": 67.0,
    "Hol": 12.8,
    "Imm": 42.0,
    "Lit": 12.4,
    "Mag": 13.8,
    "Name": "Lazuli Robe",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 21.0,
    "VSPie": 2.7,
    "VSSla": 4.2,
    "VSStr": 5.3,
    "Vita": 67.0,
    "Wgt": 4.1
},{
    "Fir": 9.5,
    "Foc": 42.0,
    "Hol": 10.2,
    "Imm": 60.0,
    "Lit": 10.2,
    "Mag": 9.5,
    "Name": "Leather Armor",
    "Phy": 8.0,
    "Poi": 2.0,
    "PoiPerWgt": 0.28169014084507044,
    "Robu": 35.0,
    "VSPie": 8.8,
    "VSSla": 8.0,
    "VSStr": 9.5,
    "Vita": 50.0,
    "Wgt": 7.1
},{
    "Fir": 11.4,
    "Foc": 24.0,
    "Hol": 10.2,
    "Imm": 35.0,
    "Lit": 11.4,
    "Mag": 10.2,
    "Name": "Leyndell Knight Armor",
    "Phy": 13.5,
    "Poi": 19.0,
    "PoiPerWgt": 1.6101694915254237,
    "Robu": 55.0,
    "VSPie": 13.5,
    "VSSla": 13.5,
    "VSStr": 12.4,
    "Vita": 24.0,
    "Wgt": 11.8
},{
    "Fir": 11.0,
    "Foc": 21.0,
    "Hol": 9.8,
    "Imm": 32.0,
    "Lit": 11.0,
    "Mag": 9.8,
    "Name": "Leyndell Knight Armor (Altered)",
    "Phy": 13.1,
    "Poi": 17.0,
    "PoiPerWgt": 1.574074074074074,
    "Robu": 50.0,
    "VSPie": 13.1,
    "VSSla": 13.1,
    "VSStr": 12.0,
    "Vita": 21.0,
    "Wgt": 10.8
},{
    "Fir": 14.5,
    "Foc": 38.0,
    "Hol": 13.1,
    "Imm": 57.0,
    "Lit": 12.6,
    "Mag": 13.1,
    "Name": "Lionel's Armor (Altered)",
    "Phy": 17.1,
    "Poi": 33.0,
    "PoiPerWgt": 1.6336633663366338,
    "Robu": 90.0,
    "VSPie": 19.3,
    "VSSla": 18.8,
    "VSStr": 15.9,
    "Vita": 45.0,
    "Wgt": 20.2
},{
    "Fir": 12.0,
    "Foc": 47.0,
    "Hol": 13.4,
    "Imm": 57.0,
    "Lit": 12.0,
    "Mag": 9.6,
    "Name": "Lord of Blood's Robe (Altered)",
    "Phy": 8.9,
    "Poi": 7.0,
    "PoiPerWgt": 0.9589041095890412,
    "Robu": 19.0,
    "VSPie": 8.1,
    "VSSla": 8.1,
    "VSStr": 9.6,
    "Vita": 47.0,
    "Wgt": 7.3
},{
    "Fir": 12.8,
    "Foc": 83.0,
    "Hol": 13.5,
    "Imm": 50.0,
    "Lit": 13.3,
    "Mag": 15.4,
    "Name": "Lusat's Robe",
    "Phy": 8.0,
    "Poi": 7.0,
    "PoiPerWgt": 0.9859154929577465,
    "Robu": 25.0,
    "VSPie": 6.1,
    "VSSla": 8.0,
    "VSStr": 6.1,
    "Vita": 99.0,
    "Wgt": 7.1
},{
    "Fir": 7.1,
    "Foc": 11.0,
    "Hol": 7.1,
    "Imm": 38.0,
    "Lit": 4.2,
    "Mag": 6.1,
    "Name": "Malenia's Armor",
    "Phy": 10.9,
    "Poi": 9.0,
    "PoiPerWgt": 1.1688311688311688,
    "Robu": 32.0,
    "VSPie": 9.5,
    "VSSla": 11.4,
    "VSStr": 7.1,
    "Vita": 11.0,
    "Wgt": 7.7
},{
    "Fir": 6.5,
    "Foc": 0.0,
    "Hol": 6.5,
    "Imm": 29.0,
    "Lit": 2.5,
    "Mag": 5.1,
    "Name": "Malenia's Armor (Altered)",
    "Phy": 10.0,
    "Poi": 7.0,
    "PoiPerWgt": 1.0294117647058825,
    "Robu": 25.0,
    "VSPie": 8.6,
    "VSSla": 10.7,
    "VSStr": 6.5,
    "Vita": 0.0,
    "Wgt": 6.8
},{
    "Fir": 10.9,
    "Foc": 21.0,
    "Hol": 12.6,
    "Imm": 29.0,
    "Lit": 8.8,
    "Mag": 10.2,
    "Name": "Maliketh's Armor (Altered)",
    "Phy": 12.9,
    "Poi": 14.0,
    "PoiPerWgt": 1.1864406779661016,
    "Robu": 50.0,
    "VSPie": 12.4,
    "VSSla": 13.5,
    "VSStr": 11.4,
    "Vita": 42.0,
    "Wgt": 11.8
},{
    "Fir": 11.4,
    "Foc": 24.0,
    "Hol": 11.4,
    "Imm": 35.0,
    "Lit": 10.2,
    "Mag": 10.9,
    "Name": "Mausoleum Knight Armor",
    "Phy": 13.5,
    "Poi": 21.0,
    "PoiPerWgt": 1.7796610169491525,
    "Robu": 55.0,
    "VSPie": 12.9,
    "VSSla": 14.6,
    "VSStr": 11.4,
    "Vita": 23.0,
    "Wgt": 11.8
},{
    "Fir": 11.0,
    "Foc": 21.0,
    "Hol": 11.0,
    "Imm": 32.0,
    "Lit": 9.8,
    "Mag": 10.5,
    "Name": "Mausoleum Knight Armor (Altered)",
    "Phy": 13.1,
    "Poi": 18.0,
    "PoiPerWgt": 1.6666666666666665,
    "Robu": 50.0,
    "VSPie": 12.5,
    "VSSla": 14.2,
    "VSStr": 11.0,
    "Vita": 21.0,
    "Wgt": 10.8
},{
    "Fir": 10.2,
    "Foc": 23.0,
    "Hol": 10.2,
    "Imm": 32.0,
    "Lit": 8.8,
    "Mag": 10.2,
    "Name": "Mausoleum Surcoat",
    "Phy": 12.9,
    "Poi": 19.0,
    "PoiPerWgt": 1.7924528301886793,
    "Robu": 50.0,
    "VSPie": 12.4,
    "VSSla": 13.5,
    "VSStr": 10.9,
    "Vita": 21.0,
    "Wgt": 10.6
},{
    "Fir": 11.4,
    "Foc": 55.0,
    "Hol": 12.4,
    "Imm": 63.0,
    "Lit": 11.9,
    "Mag": 12.4,
    "Name": "Night Maiden Armor",
    "Phy": 8.0,
    "Poi": 6.0,
    "PoiPerWgt": 0.7228915662650601,
    "Robu": 38.0,
    "VSPie": 10.2,
    "VSSla": 8.0,
    "VSStr": 9.5,
    "Vita": 55.0,
    "Wgt": 8.3
},{
    "Fir": 12.6,
    "Foc": 24.0,
    "Hol": 12.6,
    "Imm": 38.0,
    "Lit": 10.9,
    "Mag": 10.9,
    "Name": "Night's Cavalry Armor",
    "Phy": 14.0,
    "Poi": 21.0,
    "PoiPerWgt": 1.640625,
    "Robu": 57.0,
    "VSPie": 13.5,
    "VSSla": 14.0,
    "VSStr": 12.9,
    "Vita": 24.0,
    "Wgt": 12.8
},{
    "Fir": 12.4,
    "Foc": 21.0,
    "Hol": 12.4,
    "Imm": 32.0,
    "Lit": 10.2,
    "Mag": 10.2,
    "Name": "Night's Cavalry Armor (Altered)",
    "Phy": 13.5,
    "Poi": 17.0,
    "PoiPerWgt": 1.4406779661016949,
    "Robu": 50.0,
    "VSPie": 12.4,
    "VSSla": 13.5,
    "VSStr": 12.4,
    "Vita": 21.0,
    "Wgt": 11.8
},{
    "Fir": 9.5,
    "Foc": 71.0,
    "Hol": 8.8,
    "Imm": 57.0,
    "Lit": 9.5,
    "Mag": 9.5,
    "Name": "Nomadic Merchant's Finery",
    "Phy": 8.0,
    "Poi": 3.0,
    "PoiPerWgt": 0.41666666666666663,
    "Robu": 32.0,
    "VSPie": 6.7,
    "VSSla": 7.1,
    "VSStr": 8.8,
    "Vita": 46.0,
    "Wgt": 7.2
},{
    "Fir": 8.7,
    "Foc": 61.0,
    "Hol": 7.9,
    "Imm": 50.0,
    "Lit": 8.7,
    "Mag": 8.7,
    "Name": "Nomadic Merchant's Finery (Altered)",
    "Phy": 7.0,
    "Poi": 1.0,
    "PoiPerWgt": 0.1639344262295082,
    "Robu": 25.0,
    "VSPie": 6.0,
    "VSSla": 6.6,
    "VSStr": 7.9,
    "Vita": 38.0,
    "Wgt": 6.1
},{
    "Fir": 11.7,
    "Foc": 50.0,
    "Hol": 10.0,
    "Imm": 63.0,
    "Lit": 11.7,
    "Mag": 12.2,
    "Name": "Nox Monk Armor",
    "Phy": 9.3,
    "Poi": 6.0,
    "PoiPerWgt": 0.7407407407407408,
    "Robu": 42.0,
    "VSPie": 8.6,
    "VSSla": 9.3,
    "VSStr": 10.0,
    "Vita": 50.0,
    "Wgt": 8.1
},{
    "Fir": 11.2,
    "Foc": 42.0,
    "Hol": 9.3,
    "Imm": 54.0,
    "Lit": 11.2,
    "Mag": 11.7,
    "Name": "Nox Monk Armor (Altered)",
    "Phy": 8.6,
    "Poi": 4.0,
    "PoiPerWgt": 0.5405405405405405,
    "Robu": 34.0,
    "VSPie": 7.8,
    "VSSla": 8.6,
    "VSStr": 9.3,
    "Vita": 42.0,
    "Wgt": 7.4
},{
    "Fir": 10.9,
    "Foc": 55.0,
    "Hol": 11.4,
    "Imm": 60.0,
    "Lit": 12.4,
    "Mag": 12.6,
    "Name": "Nox Swordstress Armor",
    "Phy": 8.8,
    "Poi": 8.0,
    "PoiPerWgt": 0.9638554216867469,
    "Robu": 38.0,
    "VSPie": 9.5,
    "VSSla": 8.8,
    "VSStr": 10.2,
    "Vita": 55.0,
    "Wgt": 8.3
},{
    "Fir": 10.2,
    "Foc": 49.0,
    "Hol": 10.9,
    "Imm": 56.0,
    "Lit": 11.9,
    "Mag": 12.4,
    "Name": "Nox Swordstress Armor (Altered)",
    "Phy": 8.0,
    "Poi": 5.0,
    "PoiPerWgt": 0.6666666666666666,
    "Robu": 34.0,
    "VSPie": 8.8,
    "VSSla": 8.0,
    "VSStr": 9.5,
    "Vita": 49.0,
    "Wgt": 7.5
},{
    "Fir": 11.9,
    "Foc": 61.0,
    "Hol": 12.8,
    "Imm": 38.0,
    "Lit": 12.4,
    "Mag": 12.8,
    "Name": "Official's Attire",
    "Phy": 4.2,
    "Poi": 1.0,
    "PoiPerWgt": 0.24390243902439027,
    "Robu": 22.0,
    "VSPie": 5.3,
    "VSSla": 4.2,
    "VSStr": 4.2,
    "Vita": 64.0,
    "Wgt": 4.1
},{
    "Fir": 14.5,
    "Foc": 57.0,
    "Hol": 13.8,
    "Imm": 67.0,
    "Lit": 14.9,
    "Mag": 13.0,
    "Name": "Omen Armor",
    "Phy": 18.3,
    "Poi": 40.0,
    "PoiPerWgt": 1.7316017316017316,
    "Robu": 63.0,
    "VSPie": 18.7,
    "VSSla": 18.7,
    "VSStr": 15.0,
    "Vita": 76.0,
    "Wgt": 23.1
},{
    "Fir": 12.6,
    "Foc": 83.0,
    "Hol": 13.0,
    "Imm": 42.0,
    "Lit": 13.0,
    "Mag": 12.8,
    "Name": "Page Garb",
    "Phy": 6.7,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 23.0,
    "VSPie": 5.3,
    "VSSla": 5.3,
    "VSStr": 6.1,
    "Vita": 83.0,
    "Wgt": 5.1
},{
    "Fir": 12.6,
    "Foc": 64.0,
    "Hol": 13.0,
    "Imm": 34.0,
    "Lit": 13.0,
    "Mag": 12.8,
    "Name": "Page Garb (Altered)",
    "Phy": 6.3,
    "Poi": 1.0,
    "PoiPerWgt": 0.2222222222222222,
    "Robu": 19.0,
    "VSPie": 4.4,
    "VSSla": 4.4,
    "VSStr": 5.5,
    "Vita": 64.0,
    "Wgt": 4.5
},{
    "Fir": 12.4,
    "Foc": 71.0,
    "Hol": 13.0,
    "Imm": 63.0,
    "Lit": 12.6,
    "Mag": 13.0,
    "Name": "Perfumer Robe",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 21.0,
    "VSPie": 5.3,
    "VSSla": 6.1,
    "VSStr": 6.7,
    "Vita": 76.0,
    "Wgt": 5.1
},{
    "Fir": 11.9,
    "Foc": 57.0,
    "Hol": 12.8,
    "Imm": 54.0,
    "Lit": 12.4,
    "Mag": 12.8,
    "Name": "Perfumer Robe (Altered)",
    "Phy": 4.2,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 18.0,
    "VSPie": 4.2,
    "VSSla": 5.3,
    "VSStr": 6.1,
    "Vita": 67.0,
    "Wgt": 4.1
},{
    "Fir": 12.8,
    "Foc": 67.0,
    "Hol": 12.8,
    "Imm": 55.0,
    "Lit": 12.4,
    "Mag": 12.8,
    "Name": "Perfumer's Traveling Garb (Altered)",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 21.0,
    "VSPie": 2.7,
    "VSSla": 4.2,
    "VSStr": 5.3,
    "Vita": 63.0,
    "Wgt": 4.1
},{
    "Fir": 12.4,
    "Foc": 57.0,
    "Hol": 12.4,
    "Imm": 38.0,
    "Lit": 11.9,
    "Mag": 12.8,
    "Name": "Preceptor's Long Gown (Altered)",
    "Phy": 4.2,
    "Poi": 0.0,
    "PoiPerWgt": 0.0,
    "Robu": 16.0,
    "VSPie": 4.2,
    "VSSla": 2.7,
    "VSStr": 4.2,
    "Vita": 57.0,
    "Wgt": 3.2
},{
    "Fir": 12.4,
    "Foc": 76.0,
    "Hol": 12.8,
    "Imm": 38.0,
    "Lit": 12.8,
    "Mag": 13.3,
    "Name": "Prophet Robe (Altered)",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 22.0,
    "VSPie": 6.1,
    "VSSla": 5.3,
    "VSStr": 6.1,
    "Vita": 75.0,
    "Wgt": 5.1
},{
    "Fir": 14.0,
    "Foc": 34.0,
    "Hol": 13.3,
    "Imm": 52.0,
    "Lit": 12.4,
    "Mag": 13.3,
    "Name": "Radahn's Lion Armor (Altered)",
    "Phy": 18.5,
    "Poi": 24.0,
    "PoiPerWgt": 1.4634146341463417,
    "Robu": 82.0,
    "VSPie": 17.0,
    "VSSla": 17.7,
    "VSStr": 14.6,
    "Vita": 32.0,
    "Wgt": 16.4
},{
    "Fir": 11.4,
    "Foc": 55.0,
    "Hol": 10.9,
    "Imm": 63.0,
    "Lit": 11.9,
    "Mag": 10.9,
    "Name": "Ragged Armor",
    "Phy": 9.5,
    "Poi": 8.0,
    "PoiPerWgt": 0.8695652173913044,
    "Robu": 42.0,
    "VSPie": 9.5,
    "VSSla": 9.5,
    "VSStr": 10.2,
    "Vita": 55.0,
    "Wgt": 9.2
},{
    "Fir": 11.2,
    "Foc": 50.0,
    "Hol": 10.5,
    "Imm": 60.0,
    "Lit": 11.7,
    "Mag": 10.5,
    "Name": "Ragged Armor (Altered)",
    "Phy": 9.1,
    "Poi": 7.0,
    "PoiPerWgt": 0.8235294117647058,
    "Robu": 38.0,
    "VSPie": 9.1,
    "VSSla": 9.1,
    "VSStr": 9.8,
    "Vita": 50.0,
    "Wgt": 8.5
},{
    "Fir": 10.8,
    "Foc": 18.0,
    "Hol": 8.7,
    "Imm": 25.0,
    "Lit": 6.6,
    "Mag": 8.7,
    "Name": "Raging Wolf Armor (Altered)",
    "Phy": 12.8,
    "Poi": 15.0,
    "PoiPerWgt": 1.6666666666666667,
    "Robu": 50.0,
    "VSPie": 12.3,
    "VSSla": 12.8,
    "VSStr": 10.8,
    "Vita": 10.0,
    "Wgt": 9.0
},{
    "Fir": 10.2,
    "Foc": 60.0,
    "Hol": 9.5,
    "Imm": 57.0,
    "Lit": 10.9,
    "Mag": 10.2,
    "Name": "Raptor's Black Feathers",
    "Phy": 8.0,
    "Poi": 5.0,
    "PoiPerWgt": 0.6493506493506493,
    "Robu": 35.0,
    "VSPie": 9.5,
    "VSSla": 9.5,
    "VSStr": 8.8,
    "Vita": 60.0,
    "Wgt": 7.7
},{
    "Fir": 12.8,
    "Foc": 14.0,
    "Hol": 10.9,
    "Imm": 35.0,
    "Lit": 9.5,
    "Mag": 10.9,
    "Name": "Redmane Knight Armor",
    "Phy": 13.5,
    "Poi": 19.0,
    "PoiPerWgt": 1.6101694915254237,
    "Robu": 55.0,
    "VSPie": 12.4,
    "VSSla": 13.5,
    "VSStr": 11.4,
    "Vita": 25.0,
    "Wgt": 11.8
},{
    "Fir": 12.4,
    "Foc": 24.0,
    "Hol": 10.5,
    "Imm": 32.0,
    "Lit": 9.1,
    "Mag": 10.5,
    "Name": "Redmane Knight Armor (Altered)",
    "Phy": 13.1,
    "Poi": 17.0,
    "PoiPerWgt": 1.574074074074074,
    "Robu": 50.0,
    "VSPie": 12.0,
    "VSSla": 13.1,
    "VSStr": 11.0,
    "Vita": 22.0,
    "Wgt": 10.8
},{
    "Fir": 11.4,
    "Foc": 23.0,
    "Hol": 9.5,
    "Imm": 32.0,
    "Lit": 8.0,
    "Mag": 10.2,
    "Name": "Redmane Surcoat",
    "Phy": 13.5,
    "Poi": 16.0,
    "PoiPerWgt": 1.509433962264151,
    "Robu": 50.0,
    "VSPie": 11.9,
    "VSSla": 12.4,
    "VSStr": 10.9,
    "Vita": 24.0,
    "Wgt": 10.6
},{
    "Fir": 12.4,
    "Foc": 52.0,
    "Hol": 11.9,
    "Imm": 66.0,
    "Lit": 13.4,
    "Mag": 11.9,
    "Name": "Ronin's Armor",
    "Phy": 10.5,
    "Poi": 6.0,
    "PoiPerWgt": 0.7058823529411765,
    "Robu": 48.0,
    "VSPie": 10.5,
    "VSSla": 11.2,
    "VSStr": 10.5,
    "Vita": 57.0,
    "Wgt": 8.5
},{
    "Fir": 10.9,
    "Foc": 52.0,
    "Hol": 10.2,
    "Imm": 54.0,
    "Lit": 11.9,
    "Mag": 10.2,
    "Name": "Ronin's Armor (Altered)",
    "Phy": 8.8,
    "Poi": 6.0,
    "PoiPerWgt": 0.7894736842105263,
    "Robu": 38.0,
    "VSPie": 8.8,
    "VSSla": 9.5,
    "VSStr": 8.8,
    "Vita": 45.0,
    "Wgt": 7.6
},{
    "Fir": 8.0,
    "Foc": 34.0,
    "Hol": 8.0,
    "Imm": 64.0,
    "Lit": 10.2,
    "Mag": 8.0,
    "Name": "Rotten Gravekeeper Cloak (Altered)",
    "Phy": 6.1,
    "Poi": 1.0,
    "PoiPerWgt": 0.19607843137254904,
    "Robu": 25.0,
    "VSPie": 6.1,
    "VSSla": 6.1,
    "VSStr": 6.1,
    "Vita": 34.0,
    "Wgt": 5.1
},{
    "Fir": 13.3,
    "Foc": 35.0,
    "Hol": 13.0,
    "Imm": 50.0,
    "Lit": 12.4,
    "Mag": 14.1,
    "Name": "Royal Knight Armor",
    "Phy": 16.0,
    "Poi": 24.0,
    "PoiPerWgt": 1.5483870967741935,
    "Robu": 67.0,
    "VSPie": 16.8,
    "VSSla": 17.5,
    "VSStr": 14.4,
    "Vita": 35.0,
    "Wgt": 15.5
},{
    "Fir": 13.1,
    "Foc": 32.0,
    "Hol": 12.8,
    "Imm": 45.0,
    "Lit": 12.2,
    "Mag": 13.9,
    "Name": "Royal Knight Armor (Altered)",
    "Phy": 15.8,
    "Poi": 21.0,
    "PoiPerWgt": 1.4,
    "Robu": 61.0,
    "VSPie": 16.6,
    "VSSla": 17.3,
    "VSStr": 14.2,
    "Vita": 32.0,
    "Wgt": 15.0
},{
    "Fir": 10.2,
    "Foc": 24.0,
    "Hol": 8.8,
    "Imm": 35.0,
    "Lit": 8.0,
    "Mag": 9.5,
    "Name": "Royal Remains Armor",
    "Phy": 11.9,
    "Poi": 15.0,
    "PoiPerWgt": 1.4150943396226416,
    "Robu": 57.0,
    "VSPie": 11.9,
    "VSSla": 12.9,
    "VSStr": 11.4,
    "Vita": 11.0,
    "Wgt": 10.6
},{
    "Fir": 13.3,
    "Foc": 29.0,
    "Hol": 12.8,
    "Imm": 45.0,
    "Lit": 12.4,
    "Mag": 12.8,
    "Name": "Scaled Armor (Altered)",
    "Phy": 15.3,
    "Poi": 22.0,
    "PoiPerWgt": 1.4666666666666666,
    "Robu": 64.0,
    "VSPie": 15.3,
    "VSSla": 16.0,
    "VSStr": 13.4,
    "Vita": 29.0,
    "Wgt": 15.0
},{
    "Fir": 11.9,
    "Foc": 55.0,
    "Hol": 10.9,
    "Imm": 63.0,
    "Lit": 11.9,
    "Mag": 10.9,
    "Name": "Scarlet Tabard",
    "Phy": 10.2,
    "Poi": 8.0,
    "PoiPerWgt": 0.9638554216867469,
    "Robu": 42.0,
    "VSPie": 8.8,
    "VSSla": 8.8,
    "VSStr": 9.5,
    "Vita": 55.0,
    "Wgt": 8.3
},{
    "Fir": 7.1,
    "Foc": 55.0,
    "Hol": 6.7,
    "Imm": 63.0,
    "Lit": 8.0,
    "Mag": 8.0,
    "Name": "Shaman Furs",
    "Phy": 5.3,
    "Poi": 2.0,
    "PoiPerWgt": 0.3921568627450981,
    "Robu": 42.0,
    "VSPie": 8.0,
    "VSSla": 5.3,
    "VSStr": 6.1,
    "Vita": 35.0,
    "Wgt": 5.1
},{
    "Fir": 13.2,
    "Foc": 57.0,
    "Hol": 13.2,
    "Imm": 32.0,
    "Lit": 13.0,
    "Mag": 13.2,
    "Name": "Snow Witch Robe (Altered)",
    "Phy": 4.6,
    "Poi": 0.0,
    "PoiPerWgt": 0.0,
    "Robu": 25.0,
    "VSPie": 3.1,
    "VSSla": 5.7,
    "VSStr": 5.7,
    "Vita": 57.0,
    "Wgt": 4.6
},{
    "Fir": 11.3,
    "Foc": 59.0,
    "Hol": 12.7,
    "Imm": 35.0,
    "Lit": 11.8,
    "Mag": 12.7,
    "Name": "Spellblade's Traveling Attire",
    "Phy": 4.1,
    "Poi": 1.0,
    "PoiPerWgt": 0.30303030303030304,
    "Robu": 17.0,
    "VSPie": 4.1,
    "VSSla": 4.1,
    "VSStr": 2.6,
    "Vita": 63.0,
    "Wgt": 3.3
},{
    "Fir": 10.9,
    "Foc": 54.0,
    "Hol": 12.6,
    "Imm": 32.0,
    "Lit": 11.4,
    "Mag": 12.6,
    "Name": "Spellblade's Traveling Attire (Altered)",
    "Phy": 2.6,
    "Poi": 0.0,
    "PoiPerWgt": 0.0,
    "Robu": 10.0,
    "VSPie": 2.6,
    "VSSla": 2.6,
    "VSStr": 0.5,
    "Vita": 57.0,
    "Wgt": 2.5
},{
    "Fir": 12.6,
    "Foc": 67.0,
    "Hol": 12.4,
    "Imm": 35.0,
    "Lit": 12.8,
    "Mag": 13.3,
    "Name": "Traveler's Clothes",
    "Phy": 6.1,
    "Poi": 2.0,
    "PoiPerWgt": 0.48780487804878053,
    "Robu": 24.0,
    "VSPie": 2.7,
    "VSSla": 4.2,
    "VSStr": 5.3,
    "Vita": 67.0,
    "Wgt": 4.1
},{
    "Fir": 12.7,
    "Foc": 61.0,
    "Hol": 13.1,
    "Imm": 38.0,
    "Lit": 12.9,
    "Mag": 13.1,
    "Name": "Traveling Maiden Robe (Altered)",
    "Phy": 4.3,
    "Poi": 1.0,
    "PoiPerWgt": 0.23255813953488372,
    "Robu": 21.0,
    "VSPie": 2.8,
    "VSSla": 5.4,
    "VSStr": 5.4,
    "Vita": 64.0,
    "Wgt": 4.3
},{
    "Fir": 17.1,
    "Foc": 42.0,
    "Hol": 14.1,
    "Imm": 67.0,
    "Lit": 12.6,
    "Mag": 13.0,
    "Name": "Tree Sentinel Armor",
    "Phy": 18.7,
    "Poi": 28.0,
    "PoiPerWgt": 1.4814814814814816,
    "Robu": 90.0,
    "VSPie": 17.5,
    "VSSla": 18.7,
    "VSStr": 15.0,
    "Vita": 46.0,
    "Wgt": 18.9
},{
    "Fir": 16.4,
    "Foc": 34.0,
    "Hol": 14.0,
    "Imm": 57.0,
    "Lit": 12.4,
    "Mag": 12.6,
    "Name": "Tree Sentinel Armor (Altered)",
    "Phy": 18.5,
    "Poi": 25.0,
    "PoiPerWgt": 1.3812154696132595,
    "Robu": 75.0,
    "VSPie": 17.3,
    "VSSla": 18.5,
    "VSStr": 14.8,
    "Vita": 38.0,
    "Wgt": 18.1
},{
    "Fir": 10.9,
    "Foc": 21.0,
    "Hol": 8.8,
    "Imm": 25.0,
    "Lit": 7.1,
    "Mag": 8.8,
    "Name": "Tree Surcoat",
    "Phy": 12.9,
    "Poi": 15.0,
    "PoiPerWgt": 1.6304347826086958,
    "Robu": 55.0,
    "VSPie": 11.9,
    "VSSla": 12.9,
    "VSStr": 8.8,
    "Vita": 18.0,
    "Wgt": 9.2
},{
    "Fir": 11.9,
    "Foc": 21.0,
    "Hol": 10.9,
    "Imm": 32.0,
    "Lit": 9.5,
    "Mag": 11.9,
    "Name": "Twinned Armor (Altered)",
    "Phy": 14.0,
    "Poi": 17.0,
    "PoiPerWgt": 1.328125,
    "Robu": 52.0,
    "VSPie": 12.9,
    "VSSla": 15.3,
    "VSStr": 12.9,
    "Vita": 52.0,
    "Wgt": 12.8
},{
    "Fir": 10.5,
    "Foc": 19.0,
    "Hol": 8.3,
    "Imm": 29.0,
    "Lit": 8.3,
    "Mag": 9.1,
    "Name": "Vagabond Knight Armor (Altered)",
    "Phy": 13.2,
    "Poi": 12.0,
    "PoiPerWgt": 1.25,
    "Robu": 50.0,
    "VSPie": 11.7,
    "VSSla": 12.2,
    "VSStr": 10.5,
    "Vita": 19.0,
    "Wgt": 9.6
},{
    "Fir": 13.6,
    "Foc": 38.0,
    "Hol": 12.8,
    "Imm": 54.0,
    "Lit": 12.4,
    "Mag": 13.1,
    "Name": "Veteran's Armor (Altered)",
    "Phy": 18.1,
    "Poi": 29.0,
    "PoiPerWgt": 1.686046511627907,
    "Robu": 75.0,
    "VSPie": 17.3,
    "VSSla": 18.5,
    "VSStr": 16.8,
    "Vita": 38.0,
    "Wgt": 17.2
},{
    "Fir": 10.2,
    "Foc": 50.0,
    "Hol": 10.2,
    "Imm": 67.0,
    "Lit": 10.9,
    "Mag": 10.2,
    "Name": "Vulgar Militia Armor",
    "Phy": 8.8,
    "Poi": 6.0,
    "PoiPerWgt": 0.7792207792207793,
    "Robu": 35.0,
    "VSPie": 8.0,
    "VSSla": 9.5,
    "VSStr": 10.2,
    "Vita": 50.0,
    "Wgt": 7.7
},{
    "Fir": 10.1,
    "Foc": 44.0,
    "Hol": 10.1,
    "Imm": 53.0,
    "Lit": 10.8,
    "Mag": 9.4,
    "Name": "War Surgeon Gown",
    "Phy": 7.0,
    "Poi": 5.0,
    "PoiPerWgt": 0.7246376811594203,
    "Robu": 37.0,
    "VSPie": 7.0,
    "VSSla": 8.7,
    "VSStr": 7.9,
    "Vita": 48.0,
    "Wgt": 6.9
}]

"""


class Armor:
    Name = ""
    Wgt = 0
    Phy = 0
    VSStr = 0
    VSSla = 0
    VSPie = 0
    Mag = 0
    Fir = 0
    Lit = 0
    Hol = 0
    Imm = 0
    Robu = 0
    Foc = 0
    Vita = 0
    Poi = 0
    PoiPerWgt = 0

    def toJSON(self):
        return json.dumps(self, default=lambda o: o.__dict__,
                          sort_keys=True, indent=4)

    def __init__(self, _string):
        self.Name = _string.split("</td>")[0].split("Ring ")[1].split("\" href")[0]
        self.Phy = float(_string.split("</td>")[1].split(">")[1])
        self.VSStr = float(_string.split("</td>")[2].split(">")[1])
        self.VSSla = float(_string.split("</td>")[3].split(">")[1])
        self.VSPie = float(_string.split("</td>")[4].split(">")[1])
        self.Mag = float(_string.split("</td>")[5].split(">")[1])
        self.Fir = float(_string.split("</td>")[6].split(">")[1])
        self.Lit = float(_string.split("</td>")[7].split(">")[1])
        self.Hol = float(_string.split("</td>")[8].split(">")[1])
        self.Imm = float(_string.split("</td>")[9].split(">")[1])
        self.Robu = float(_string.split("</td>")[10].split(">")[1])
        self.Foc = float(_string.split("</td>")[11].split(">")[1])
        self.Vita = float(_string.split("</td>")[12].split(">")[1])
        self.Wgt = float(_string.split("</td>")[14].split(">")[1])
        self.Poi = float(_string.split("</td>")[13].split(">")[1])
        self.PoiPerWgt = self.Poi / self.Wgt

    def showAll(self):
        msgbox.showinfo("title",
                        self.Name + "\n 重量：" +
                        str(self.Wgt) + "\n 物理：" +
                        str(self.Phy) + "\n 打击抗性：" +
                        str(self.VSStr) + "\n 斩击抗性：" +
                        str(self.VSSla) + "\n 突刺抗性：" +
                        str(self.VSPie) + "\n 魔力抗性：" +
                        str(self.Mag) + "\n 火抗性：" +
                        str(self.Fir) + "\n 雷抗性：" +
                        str(self.Lit) + "\n 圣抗性：" +
                        str(self.Hol) + "\n 免疫力：" +
                        str(self.Imm) + "\n 健壮度：" +
                        str(self.Robu) + "\n 理智度：" +
                        str(self.Foc) + "\n 抗死度：" +
                        str(self.Vita) + "\n 韧性：" +
                        str(self.Poi) + "\n 韧重比：" +
                        str(self.PoiPerWgt)
                        )


'''
    def __init__(self,_dir):
        self.Name = _dir['Name']
        self.Phy = _dir['Phy']
        self.VSStr = _dir['VSStr']
        self.VSSla = _dir['VSSla']
        self.VSPie = _dir['VSPie']
        self.Mag = _dir['Mag']
        self.Fir = _dir['Fir']
        self.Lit = _dir['Lit']
        self.Hol = _dir['Hol']
        self.Imm = _dir['Imm']
        self.Robu = _dir['Robu']
        self.Foc = _dir['Foc']
        self.Vita = _dir['Vita']
        self.Wgt = _dir['Wgt']
        self.Poi = _dir['Poi']
        self.PoiPerWgt = self.Poi / self.Wgt
'''
helmStrList = HelmText.split("</tr>")
for i in range(0, len(helmStrList) - 2):
    HelmList.append(Armor(helmStrList[i]))


chestStrList = chestText.split("</tr>")
for i in range(0, len(chestStrList) - 2):
    ChestList.append(Armor(chestStrList[i]))


gauntletStrList = GauntletText.split("</tr>")
for i in range(0, len(gauntletStrList) - 2):
    GauntletList.append(Armor(gauntletStrList[i]))

legStrList = LegText.split("</tr>")
for i in range(0, len(legStrList) - 2):
    LegList.append(Armor(legStrList[i]))


for i in range(0,len(GauntletList)):
    if i == 0:
        text = text+"["
    text = text + GauntletList[i].toJSON()
    if i < len(GauntletList)-1:
        text = text+","
    else:
        text = text+"]"

print(text)
